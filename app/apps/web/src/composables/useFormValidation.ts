...
export interface ValidationRule<TModel = Record<string, unknown>> {
  validator: (value: unknown, model: TModel) => boolean | string | Promise<boolean | string>;
  message?: string;
}

export type ValidationRules<TModel> = Partial<Record<keyof TModel, ValidationRule<TModel>[]>>;

export function useFormValidation<TModel extends Record<string, unknown>>(
  model: TModel,
