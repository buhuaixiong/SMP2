<template>
  <div class="finance-accountant-invoice-view">
    ...
    <el-dialog
      ...
      width="800px"
    >
      <el-descriptions v-if="selectedInvoiceDetail" :column="2" border>
        <el-descriptions-item :label="t('financeInvoice.detailsDialog.invoiceNumber')">
          {{ selectedInvoiceDetail.invoice_number }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('financeInvoice.detailsDialog.supplier')">
          {{ selectedInvoiceDetail.supplier_name }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('financeInvoice.detailsDialog.issueDate')">
          {{ selectedInvoiceDetail.issue_date || selectedInvoiceDetail.invoice_date }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('financeInvoice.detailsDialog.amount')">
          ¥{{ (selectedInvoiceDetail.amount || selectedInvoiceDetail.total_amount || 0).toFixed(2) }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('financeInvoice.detailsDialog.type')">
          <el-tag :type="selectedInvoiceDetail.type === 'temporary_supplier' ? 'warning' : 'success'">
            {{
              selectedInvoiceDetail.type === "temporary_supplier"
        ...
        <el-descriptions-item :label="t('financeInvoice.detailsDialog.status')">
          <el-tag :type="getStatusType(selectedInvoiceDetail.status || 'pending')">
            {{ getStatusText(selectedInvoiceDetail.status || 'pending') }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item
          :label="t('financeInvoice.detailsDialog.taxRate')"
          v-if="selectedInvoiceDetail.tax_rate"
        >
          {{ selectedInvoiceDetail.tax_rate }}
        </el-descriptions-item>
        <el-descriptions-item :label="t('financeInvoice.detailsDialog.uploadedAt')">
          {{ selectedInvoiceDetail.uploaded_at || selectedInvoiceDetail.created_at }}
        </el-descriptions-item>
        <el-descriptions-item
          :label="t('financeInvoice.detailsDialog.reviewNotes')"
          :span="2"
          v-if="selectedInvoiceDetail.review_notes"
        >
          {{ selectedInvoiceDetail.review_notes }}
        </el-descriptions-item>
        <el-descriptions-item
          :label="t('financeInvoice.detailsDialog.rejectionReason')"
          :span="2"
          v-if="selectedInvoiceDetail.rejection_reason"
        >
          {{ selectedInvoiceDetail.rejection_reason }}
        </el-descriptions-item>
        <el-descriptions-item
          :label="t('financeInvoice.detailsDialog.validationErrors')"
          :span="2"
          v-if="selectedInvoiceDetail.validation_errors"
        >
          <el-alert type="warning" :closable="false">
            {{ selectedInvoiceDetail.validation_errors }}
          </el-alert>
        </el-descriptions-item>
      </el-descriptions>

      <el-card v-if="selectedInvoiceDetail?.file" style="margin-top: 20px">
        <template #header>{{ t("financeInvoice.detailsDialog.attachmentInfo") }}</template>
        <div class="file-info">
          <el-icon><Document /></el-icon>
          <span>{{ selectedInvoiceDetail.file.original_name }}</span>
...
interface InvoiceViewDetail extends InvoiceDetailResponse {
  issue_date?: string;
  amount?: number;
  total_amount?: number;
...
const selectedInvoice = ref<InvoiceViewDetail | null>(null);
const selectedInvoiceDetail = computed<InvoiceViewDetail | null>(() => selectedInvoice.value);
...
const submitUpload = async () => {
  ...
  await uploadFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      notification.error(extractErrorMessage(error) || t("financeInvoice.messages.uploadFailed"));
    } finally {
...
const submitReview = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("financeInvoice.messages.reviewFailed"));
  } finally {
...
const confirmDelete = async (row: Invoice) => {
  try {
    ...
    if (error !== "cancel") {
      ...
      notification.error(extractErrorMessage(error) || t("financeInvoice.messages.deleteFailed"));
    }
  }
};
...
const downloadInvoiceFile = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("financeInvoice.messages.downloadFailed"));
  }
};
