<template>
  <div class="directory-page">
    ...
    <AdvancedFiltersPanel
      ...
      @update:stage="updateStageFilter"
      @update:category="updateCategoryFilter"
      @update:region="updateRegionFilter"
      @update:importance="updateImportanceFilter"
      @update:tag="updateTagFilter"
    />
...
import type { ApprovalRecord, FileApprovalReview, Supplier, SupplierTag } from "@/types";
...
const updateStageFilter = (value: string) => {
  filters.stage = value;
};
...
const updateRegionFilter = (value: string) => {
  filters.region = value;
};

const updateImportanceFilter = (value: string) => {
...
const filteredSuppliers = computed(() => {
  ...
  return suppliers.value.filter((supplier) => {
    ...
    if (filters.tag) {
      const tags: SupplierTag[] = Array.isArray(supplier.tags) ? supplier.tags : [];
...
const mergedApprovals = computed<TimelineItem[]>(() => {
  ...
  selectedSupplier.value.approvalHistory?.forEach((approval: ApprovalRecord) => {
    items.push({
      key: `status_${approval.step}_${approval.date}`,
      source: 'status_approval',
      title: approval.step || '',
      statusLabel: approval.result === 'approved' ? t('common.approved') : t('common.rejected'),
      actor: approval.approver || '',
      decidedAt: approval.date || '',
  ...
  // Add file approvals
  selectedSupplier.value.fileApprovals?.forEach((approval: FileApprovalReview) => {
...
let filterFetchTimeout: number | null = null;
...
watch(
  ...
  async () => {
    ...
    filterFetchTimeout = window.setTimeout(() => {
      void supplierStore.fetchSuppliers(buildFilterPayload(), {
        page: 1,
        pageSize: pageSizeValue.value,
        force: true,
      });
