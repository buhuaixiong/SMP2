          ...
          <div class="material-type-selection">
            <el-radio-group v-model="formData.materialCategoryType" size="large">
              <el-card
                ...
                :class="{ selected: formData.materialCategoryType === RfqMaterialType.IDM }"
                shadow="hover"
                @click="formData.materialCategoryType = RfqMaterialType.IDM"
              >
                <el-radio value="IDM">
              ...
              <el-card
                class="material-type-card"
                :class="{ selected: formData.materialCategoryType === RfqMaterialType.DM }"
                shadow="hover"
                @click="formData.materialCategoryType = RfqMaterialType.DM"
              >
...
import {
  createRfqWorkflow,
  importRfqFromExcel,
  type RfqWorkflowCreatePayload,
} from "@/api/rfq";
import { markItemAsConverted } from "@/api/requisitions";
import { RfqMaterialType, type ExternalEmailInvitation, type RfqType } from "@/types";
import { useNotification, useFileUpload } from "@/composables";
import { extractErrorMessage } from "@/utils/errorHandling";

interface RfqCreateFormData {
  materialCategoryType: RfqMaterialType;
  title: string;
  description?: string;
  rfqType: string;
...
const formData = reactive<RfqCreateFormData>({
  // Material category type (IDM/DM)
  materialCategoryType: RfqMaterialType.IDM,
  // Line items mode
...
async function nextStep() {
  if (currentStep.value === 0) {
    ...
    if (formData.materialCategoryType === RfqMaterialType.DM) {
...
async function submitRfq() {
  ...
  try {
    ...
    const payload = {
      ...
      externalEmails: formData.externalEmails.map((item) => item.email),
      minSupplierCount: formData.minSupplierCount,
      supplierExceptionNote: formData.supplierExceptionNote,
      requiredDocuments: formData.requiredDocuments,
      evaluationCriteria: formData.evaluationCriteria,
    } satisfies RfqWorkflowCreatePayload & { projectLocation: string };
    ...
    notification.error(extractErrorMessage(error) || t("rfq.create.error"));
  } finally {
...
async function handleExcelFileSelect(file: UploadFile) {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("rfq.import.error"));
  } finally {
    submitting.value = false;
    // 清除上传组件状态
    excelUploadRef.value?.clearFiles();
