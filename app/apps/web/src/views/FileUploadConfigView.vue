...
async function loadConfigs() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("fileUpload.messages.loadFailed"));
  } finally {
    loading.value = false;
  }
}
...
async function handleSubmit() {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      notification.error(extractErrorMessage(error) || t("fileUpload.messages.operationFailed"));
    } finally {
...
async function handleDelete(row: FileUploadConfig) {
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || t("fileUpload.messages.deleteFailed"));
    }
  }
}
