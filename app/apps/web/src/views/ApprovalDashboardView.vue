...
type DashboardFile = UpgradeApplicationDocument["file"];
...
const approvalHistory = ref<UpgradeReview[]>([]);
...
const loadApplications = async () => {
  loading.value = true;
  try {
    const data = await fetchPendingUpgradeApplicationsPaged({
      page: 1,
      pageSize: 50,
    });
    applications.value = data;
  } catch (error: unknown) {
    notification.error(extractErrorMessage(error) || t("approvalDashboard.loadError"));
  } finally {
    loading.value = false;
  }
};
...
const viewApplication = async (
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("approvalDashboard.loadDetailError"));
  } finally {
...
const canApprove = (app: PendingUpgradeApplication): boolean => {
  ...
  return requiredPermission ? Boolean(authStore.user.permissions?.includes(requiredPermission)) : false;
};
...
const submitDecision = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("approvalDashboard.decisionError"));
  } finally {
...
const downloadDocument = async (file: DashboardFile) => {
  if (!file?.id) {
    return;
  }
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("approvalDashboard.messages.fileNotFound"));
  }
};
