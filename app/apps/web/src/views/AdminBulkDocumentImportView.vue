    ...
    <el-card v-if="uploadResults.length > 0" class="results-card" shadow="never">
      ...
      <div class="results-summary">
        ...
        <el-statistic
          ...
          :value="successfulUploadCount"
          value-style="color: #67c23a"
        />
        <el-statistic
          title="Failed Uploads"
          :value="failedUploadCount"
...
interface HistoryStateWithPreselectedSuppliers {
  preSelectedSupplierIds?: number[];
}
...
const preSelectedIds = (history.state as HistoryStateWithPreselectedSuppliers | null)
...
const successfulUploadCount = computed(
  () => uploadResults.value.filter((result) => result.successCount > 0).length,
);
const failedUploadCount = computed(
  () => uploadResults.value.filter((result) => result.failedCount > 0).length,
);
