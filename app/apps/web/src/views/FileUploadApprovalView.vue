...
const fetchPendingApprovals = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "获取待审批列表失败");
  } finally {
    loading.value = false;
  }
};
...
const fetchApprovedApprovals = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "获取已审批列表失败");
  } finally {
...
const downloadFile = async (fileId: number) => {
  ...
  try {
    await openFileInNewTab(downloadUrl);
  } catch (error: unknown) {
...
const handleSubmitApproval = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "审批提交失败");
  } finally {
...
const handleSendReminder = async (uploadId: number) => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "发送提醒邮件失败");
  }
};
