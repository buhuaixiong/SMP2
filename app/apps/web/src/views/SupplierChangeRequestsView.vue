...
import type { ChangeRequest, ChangeRequestDetails, ChangeRequestFieldChange } from "@/api/changeRequests";
...
const loadRequests = async () => {
  ...
  try {
    loading.value = true;
    requests.value = await getSupplierChangeRequests(currentSupplierId.value);
  } catch (error: unknown) {
    console.error("加载变更申请失败:", error);
    notification.error(extractErrorMessage(error) || "加载失败");
  } finally {
    loading.value = false;
  }
};
...
const viewDetails = async (request: ChangeRequest) => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "加载详情失败");
  }
};
...
// 获取变更字段数量
const getChangedFieldsCount = (payload?: Record<string, unknown> | null) => {
  return Object.keys(payload || {}).length;
};

// 获取变更字段数组
const getChangedFieldsArray = (payload?: Record<string, unknown> | null): ChangeRequestFieldChange[] => {
  if (!payload) return [];
...
const loadSupplierData = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "加载供应商数据失败");
  } finally {
...
const handleSubmitChangeRequest = async () => {
  if (currentSupplierId.value == null || !originalSupplierData.value) {
    notification.error("无法获取供应商信息");
    return;
  }
  ...
  try {
    createFormLoading.value = true;
    const result = await submitChangeRequest(currentSupplierId.value, changes);
    ...
    notification.error(extractErrorMessage(error) || "提交失败");
  } finally {
    createFormLoading.value = false;
  }
};
