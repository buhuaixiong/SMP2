...
const submitApproval = async () => {
  ...
  try {
    ...
    if (error !== "cancel") {
      ...
      notification.error(extractErrorMessage(error) || "审批失败");
    }
  } finally {
...
const getChangedFieldsCount = (payload: Record<string, unknown> | null | undefined) => {
  return Object.keys(payload || {}).length;
};

const getChangedFieldsArray = (payload: Record<string, unknown> | null | undefined) => {
  if (!payload) return [];
