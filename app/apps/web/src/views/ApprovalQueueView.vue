    ...
    >
      ...
      <div v-else class="drawer-content">
        ...
        <el-descriptions :column="2" border>
          ...
          <el-descriptions-item label="发票类型" v-if="displayInvoiceType">
            {{ displayInvoiceType }}
          </el-descriptions-item>
          <el-descriptions-item label="付款条款" v-if="applicationDetail.paymentTermsDays">
            {{ applicationDetail.paymentTermsDays }} 天
          </el-descriptions-item>
          <el-descriptions-item label="付款方式" :span="2" v-if="displayPaymentMethods">
...
const displayInvoiceType = computed(() =>
  formatApprovalInvoiceType(applicationDetail.value?.invoiceType, (key) => String(t(key)))
);
const displayPaymentMethods = computed(() =>
  formatApprovalPaymentMethods(
    applicationDetail.value?.paymentMethods,
    applicationDetail.value?.paymentMethodsOther,
    (key) => String(t(key)),
  )
);
...
const openDocument = async (relativePath: string | null | undefined) => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "文件打开失败，请稍后重试。");
  }
};
...
const handleBindCode = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "绑定失败");
  } finally {
