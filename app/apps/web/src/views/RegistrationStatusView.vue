...
const resolvedSupplierId = computed(() => {
...
const loadStatus = async () => {
  ...
  try {
    ...
    } else if (resolvedSupplierId.value != null) {
      status.value = await fetchSupplierRegistrationStatusBySupplier(resolvedSupplierId.value);
...
watch(
  () => [resolvedApplicationId.value, resolvedSupplierId.value, authStore.user?.relatedApplicationId],
  () => {
    loadStatus();
  },
  { immediate: true },
