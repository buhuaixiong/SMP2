...
import { formatDateTime, optional, isPlainObject, translateOr as createTranslateOr, copyToClipboard as utilCopyToClipboard } from "@/utils";
import { buildDraftPayloadSnapshot, collectChangedFields } from "@/utils/formChangeTracker";
import {
...
const FORM_FIELD_KEYS = Object.keys(createDefaultSupplierRegistrationForm()) as Array<keyof FormState>;
const FORM_WATCH_GROUPS: Array<Array<keyof FormState>> = [
  [
    "companyName",
    "englishName",
    "chineseName",
...
const tempAccountQrUrl = computed(() => {
  ...
  return `${normalizedThirdPartyBaseUrl}/qr?text=${encodeURIComponent(
    `user=${tempAccount.value.username}`,
  )}`;
});
...
const requiredFieldsByStep: Array<Array<keyof FormState>> = [
  ...
  ["invoiceType", "paymentTermsDays", "paymentMethods"],
...
const buildDraftPayload = () => buildDraftPayloadSnapshot(form as unknown as Record<string, unknown>);

const clearServerFieldError = (fieldKey: keyof FormState) => {
  if (!serverFieldErrors.value[fieldKey]) {
    return;
  }
...
const markChangedField = (fieldKey: keyof FormState) => {
  const key = String(fieldKey);
  if (!changedFields.value.includes(key)) {
    changedFields.value.push(key);
  }
  clearServerFieldError(fieldKey);
};

const handleTrackedFormChanges = (changedKeys: string[]) => {
  if (suppressChangeTracking.value || !changedKeys.length) {
    return;
  }
  changedKeys.forEach((key) => markChangedField(key as keyof FormState));
  if (approvalPreview.value) {
...
const buildWatchedGroupSnapshot = (keys: Array<keyof FormState>) => {
...
const normalizePaymentTermsDays = () => normalizeRegistrationPaymentTermsDays(form.paymentTermsDays);

const paymentTermsInRange = () => {
  return isRegistrationPaymentTermsValid(form.paymentTermsDays, paymentTermCodes);
};
...
const handleSaveDraft = async ({ silent = false }: { silent?: boolean } = {}) => {
  ...
  try {
    ...
    changedFields.value = collectChangedFields(
      formPayload,
      currentSnapshot,
      FORM_FIELD_KEYS as string[],
    );
    draftError.value = null;
...
FORM_WATCH_GROUPS.forEach((group) => {
  watch(
    () => buildWatchedGroupSnapshot(group),
    (nextSnapshot, previousSnapshot) => {
      ...
      handleTrackedFormChanges(changed);
    },
  );
});
