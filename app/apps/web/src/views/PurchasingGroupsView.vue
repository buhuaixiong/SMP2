...
type BuyerOption = usersApi.User;

const availableBuyers = ref<BuyerOption[]>([]);
...
const leaderGroupIdSet = computed(() => {
  ...
  (authStore.user?.purchasingGroups ?? []).forEach((group) => {
...
const submitGroup = async () => {
  ...
  await groupFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      notification.error(extractErrorMessage(error) || fallback);
    } finally {
...
const fetchBuyers = async () => {
  try {
    ...
    availableBuyers.value = response.filter((user) => user.role === "purchaser");
  } catch (error) {
