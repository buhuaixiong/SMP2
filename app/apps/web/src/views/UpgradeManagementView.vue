...
async function handleUploadFile() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("upgradeManagement.upload.uploadFailed"));
  } finally {
...
function openPreview(url: string | null) {
  ...
  openFileInNewTab(url)
    ...
    .catch((error: unknown) => {
      notification.error(extractErrorMessage(error) || t("upgradeManagement.upload.downloadFailed"));
    });
}
...
const polling = useVisibilityAwarePolling({
  task: loadUpgradeStatus,
  shouldPoll: shouldPollUpgradeStatus,
...
onMounted(() => {
  ...
  void loadUpgradeStatus().finally(() => {
    polling.start();
  });
});
