...
const handleSave = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      notification.error(extractErrorMessage(error) || "保存配置失败");
    } finally {
      saving.value = false;
    }
  });
...
const sendTestEmail = async () => {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "发送测试邮件失败");
  } finally {
    testing.value = false;
  }
};
...
const handleDelete = async () => {
  try {
    ...
    if (error !== "cancel") {
      ...
      notification.error(extractErrorMessage(error) || "删除配置失败");
    }
  }
};
