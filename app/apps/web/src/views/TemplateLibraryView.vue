        ...
        >
          <template #actions="{ file }">
            <button
              ...
              type="button"
              class="primary-btn"
              @click="handleTemplateDownload(file.downloadUrl, file.originalName)"
            >
              {{ t("common.download") }}
            </button>
          </template>
...
const handleTemplateDownload = async (url?: string | null, originalName?: string | null) => {
  if (!url) {
    return;
  }

  try {
    await downloadFile(url, originalName || "template-file");
  } catch (error) {
