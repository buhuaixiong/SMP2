<template>
  <div class="rfq-detail-view" v-loading="loading">
    <PageHeader>
      ...
      <template #actions>
        <div class="header-actions" v-if="!isSupplier && (canEditRfq || canPreviewComparisonPrint)">
          ...
          <el-button v-if="canPreviewComparisonPrint" type="primary" plain @click="showComparisonPrintPreview = true">
            {{ t("rfq.print.trigger") }}
          </el-button>
        </div>
      </template>
    ...
    <RfqComparisonPrintPreviewDialog
      v-model="showComparisonPrintPreview"
      :rfq-id="rfq?.id ?? null"
    />

    <RfqSupplierComparisonHistoryDialog
      v-model:visible="showRoundHistoryDialog"
      :loading="roundHistoryLoading"
      :rfq-id="rfq?.id"
    ...
    <RfqExtendBidDeadlineDialog
      v-model:visible="showExtendRoundDialog"
      :rfq-id="rfq?.id ?? null"
      :current-round="rfq?.currentRound ?? null"
      :format-date-time="formatDateTime"
      @success="handleRoundActionSuccess"
    />

    <RfqStartNextRoundDialog
      v-model:visible="showStartNextRoundDialog"
      :rfq-id="rfq?.id ?? null"
      :current-round="rfq?.currentRound ?? null"
      :initial-supplier-ids="currentRoundSupplierIds"
      :format-date-time="formatDateTime"
      @success="handleRoundActionSuccess"
    />
    ...
    <RfqBidRoundHistoryCard
      v-if="rfq?.currentRound"
      :current-round="rfq.currentRound"
      :rounds="rfq.rounds || []"
      :show-history-action="canViewRoundHistory"
      :show-extend-action="canManageRoundActions"
      :show-start-next-action="canManageRoundActions"
      :extend-action-disabled="extendDeadlineDisabled"
      :start-next-action-disabled="startNextRoundDisabled"
      :format-date-time="formatDateTime"
      @view-history="openRoundHistory"
      @extend-deadline="showExtendRoundDialog = true"
      @start-next-round="showStartNextRoundDialog = true"
    />
    ...
    >
      ...
      <SupplierQuoteForm
        v-if="rfq"
        :rfq="rfq"
        :existing-quote="supplierQuote || undefined"
    ...
    <el-card v-if="!isSupplier" class="suppliers-card" shadow="never">
      ...
      <el-table :data="rfq?.invitedSuppliers" style="width: 100%">
        ...
        <el-table-column prop="quoteStatus" :label="t('rfq.detail.preBidQuoteStatus')" width="140">
          <template #default="{ row }">
            <el-tag :type="getInvitedSupplierQuoteStatusType(row.quoteStatus)" size="small">
              {{ translateQuoteStatusLabel(row.quoteStatus || "not_submitted") }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="quoteSubmittedAt" :label="t('rfq.detail.quoteSubmittedAt')" width="180">
          <template #default="{ row }">
          ...
          <el-table-column :label="t('rfq.quotes.fileName')" min-width="220">
            <template #default="{ row }">
              <el-link
                ...
                type="primary"
                @click.prevent="openAttachmentInNewTab(row)"
              >
    ...
    <el-card v-if="!isSupplier && !showLineItemWorkflow" class="quotes-card" shadow="never">
      ...
      >
        ...
        <div v-else>
          <el-alert
            v-if="rfq?.currentRound"
            :title="t('rfq.rounds.defaultComparisonLatestOnly')"
            type="info"
            :closable="false"
            class="latest-round-alert"
          />
              ...
              <el-row v-else :gutter="12">
                <el-col :span="8">
                  <input
                    type="file"
                    accept="image/*"
                    @change="handlePriceComparisonFileChange('1688', $event)"
                    style="width: 100%"
                  />
                </el-col>
                <el-col :span="10">
              ...
              <el-row v-else :gutter="12">
                <el-col :span="8">
                  <input
                    type="file"
                    accept="image/*"
                    @change="handlePriceComparisonFileChange('jd', $event)"
                    style="width: 100%"
                  />
                </el-col>
                <el-col :span="10">
              ...
              <el-row v-else :gutter="12">
                <el-col :span="8">
                  <input
                    type="file"
                    accept="image/*"
                    @change="handlePriceComparisonFileChange('zkh', $event)"
                    style="width: 100%"
                  />
                </el-col>
                <el-col :span="10">
...
import RfqComparisonPrintPreviewDialog from "@/components/RfqComparisonPrintPreviewDialog.vue";
import RfqExtendBidDeadlineDialog from "@/components/RfqExtendBidDeadlineDialog.vue";
import RfqStartNextRoundDialog from "@/components/RfqStartNextRoundDialog.vue";
import {
  QuoteStatus,
  type Rfq,
  type RfqBidRoundSummary,
  type RfqItem,
  type RfqQuoteItem,
  ...
  type RfqSupplierComparisonHistory,
} from "@/types";
import { openFileInNewTab, resolveUploadDownloadUrl } from "@/utils/fileDownload";
...
type PriceComparisonRecord = {
  ...
  bidRoundId?: number | string | null;
  bid_round_id?: number | string | null;
...
const normalizedUserRole = computed(() => String(authStore.user?.role ?? ""));

const RfqPriceComparisonTable = defineAsyncComponent(
  () => import("@/components/RfqPriceComparisonTable.vue"),
);
const RfqBidRoundHistoryCard = defineAsyncComponent(
  () => import("@/components/RfqBidRoundHistoryCard.vue"),
);
const RfqSupplierComparisonHistoryDialog = defineAsyncComponent(
  () => import("@/components/RfqSupplierComparisonHistoryDialog.vue"),
);
...
const canViewRoundHistory = computed(() => {
  const role = normalizedUserRole.value;
  return role === "purchaser" || role === "procurement_manager" || role === "procurement_director" || role === "admin";
});

const canManageRoundActions = computed(() => {
  const role = normalizedUserRole.value;
  return role === "purchaser" || role === "admin";
});
...
const showComparisonPrintPreview = ref(false);
const showRoundHistoryDialog = ref(false);
const showExtendRoundDialog = ref(false);
const showStartNextRoundDialog = ref(false);
const roundHistoryLoading = ref(false);
const roundHistory = ref<RfqSupplierComparisonHistory | null>(null);
...
const currentRoundSupplierIds = computed(() =>
  Array.isArray(rfq.value?.invitedSuppliers)
    ? rfq.value!.invitedSuppliers
        .map((supplier) => Number(supplier.id))
        .filter((id) => Number.isFinite(id) && id > 0)
    : [],
);

const extendDeadlineDisabled = computed(() => {
  if (!canManageRoundActions.value || !rfq.value?.currentRound) {
    return true;
  }

  const round = rfq.value.currentRound;
  return String(round.status ?? "").toLowerCase() !== "published" ||
    Boolean(round.opened) ||
    (round.pendingCount ?? 0) <= 0;
});

const startNextRoundDisabled = computed(() => {
  if (!canManageRoundActions.value || !rfq.value?.currentRound) {
    return true;
  }

  const round = rfq.value.currentRound;
  return !Boolean(round.opened);
});
...
async function openAttachmentInNewTab(attachment: { downloadUrl?: string | null }) {
  ...
  try {
    await openFileInNewTab(String(downloadUrl));
  } catch (error) {
...
const canSupplierEditQuote = computed(() => {
  ...
  const activeDeadline = rfq.value.currentRound?.bidDeadline || rfq.value.validUntil;
  const deadline = activeDeadline ? new Date(activeDeadline) : null;
...
const canViewQuotes = computed(() => {
  ...
  if (normalizedUserRole.value === "admin") return true;

  // For suppliers, they can always view their own quotes
  if (normalizedUserRole.value === "temp_supplier" || normalizedUserRole.value === "formal_supplier") {
    return true;
  }
  ...
  if (!authorizedRoles.includes(normalizedUserRole.value)) {
    return false;
  }
  ...
  const now = new Date();
  const visibilityDeadline = rfq.value.currentRound?.bidDeadline || rfq.value.validUntil;
  const deadline = visibilityDeadline ? new Date(visibilityDeadline) : null;
...
const canPreviewComparisonPrint = computed(() => {
  const role = normalizedUserRole.value;
  const printableRole = role === "purchaser" || role === "procurement_manager" || role === "procurement_director" || role === "admin";
  if (!printableRole || !rfq.value) {
    return false;
  }

  const selectedQuoteIdRaw = rfq.value.selectedQuoteId ?? rfq.value.selected_quote_id;
  return rfq.value.status === "closed" && !!rfq.value.reviewCompletedAt && !!selectedQuoteIdRaw;
});
...
function normalizePriceComparisonRecords(records: PriceComparisonRecord[] | undefined | null) {
  ...
  return records.map((record) => {
    ...
    const bidRoundId = record.bidRoundId ?? record.bid_round_id ?? null;
    const roundNumber = record.roundNumber ?? record.round_number ?? null;
...
function normalizeRoundSummary(round: RfqBidRoundSummary | null | undefined): RfqBidRoundSummary | null {
  if (!round) {
    return null;
  }
  ...
  return {
    ...
    invitedCount: round.invitedCount ?? 0,
    submittedCount: round.submittedCount ?? 0,
    pendingCount: round.pendingCount ?? Math.max((round.invitedCount ?? 0) - (round.submittedCount ?? 0), 0),
  };
}

function normalizeRoundSummaries(rounds: RfqBidRoundSummary[] | undefined | null): RfqBidRoundSummary[] {
  if (!Array.isArray(rounds)) {
    return [];
  }

  return rounds
    .map((round) => normalizeRoundSummary(round))
    .filter((round): round is RfqBidRoundSummary => !!round);
}
...
async function loadRfq() {
  ...
  try {
    ...
    baseData.currentRound = normalizeRoundSummary(baseData.currentRound);
    baseData.rounds = normalizeRoundSummaries(baseData.rounds);
...
async function loadRoundHistory() {
  if (!rfq.value?.id || !canViewRoundHistory.value) {
    return;
  }

  roundHistoryLoading.value = true;
  try {
    const data = await fetchRfqSupplierComparisonHistory(rfq.value.id);
    roundHistory.value = {
      ...data,
      rounds: Array.isArray(data.rounds)
        ? data.rounds.map((round) => ({
            ...round,
            summary: normalizeRoundSummary(round.summary),
            priceComparisons: normalizePriceComparisonRecords(round.priceComparisons as unknown as PriceComparisonRecord[]),
          }))
        : [],
    };
  } catch (error) {
    ...
    roundHistoryLoading.value = false;
  }
}

async function openRoundHistory() {
  showRoundHistoryDialog.value = true;
  if (!roundHistory.value) {
    await loadRoundHistory();
  }
}

async function handleRoundActionSuccess() {
  roundHistory.value = null;
  showRoundHistoryDialog.value = false;
  await loadRfq();
}
...
function translateQuoteStatusLabel(status: string | null | undefined): string {
  if (!status) return t("rfq.quoteStatus.not_submitted");
...
function getDeadlineText(): string {
  const deadlineValue = rfq.value?.currentRound?.bidDeadline || rfq.value?.validUntil;
