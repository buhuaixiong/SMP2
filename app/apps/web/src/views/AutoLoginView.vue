...
import { getPostAutoLoginNavigation } from "@/utils/authPasswordChangeRouting";
...
onMounted(async () => {
  ...
  try {
    ...
    if (res.data.success) {
      ...
      const postLoginNavigation = getPostAutoLoginNavigation(
        authStore.user,
      ...
      setTimeout(() => {
        router.push(postLoginNavigation);
