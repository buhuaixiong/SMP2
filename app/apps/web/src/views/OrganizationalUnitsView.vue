...
type CascaderUnitNode = OrganizationalUnit & {
  disabled?: boolean;
  children?: CascaderUnitNode[];
};
...
const moveParentOptions = computed(() => {
  ...
  const clone = (nodes: OrganizationalUnit[]): CascaderUnitNode[] =>
    nodes.map((node) => ({
      ...node,
...
async function ensureUserOptions(force = false) {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "加载用户列表失败");
  } finally {
...
async function submitUnit() {
  ...
  await unitFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      notification.error(extractErrorMessage(error) || "操作失败");
    } finally {
...
async function confirmDelete(unit: OrganizationalUnit) {
  ...
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || "删除失败");
    }
  }
}
...
async function confirmMove() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "移动失败");
  } finally {
    moveSubmitting.value = false;
  }
}
...
async function confirmAddMembers() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "添加失败");
  } finally {
...
async function saveAdminAssignment() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "更新失败");
  } finally {
