...
const handleUpdateRates = async () => {
  try {
    ...
    if (error !== 'cancel') {
      notification.error(extractErrorMessage(error) || t('exchangeRate.updateFailed'));
...
const handleDelete = async (record: ExchangeRate) => {
  try {
    ...
    if (error !== 'cancel') {
      notification.error(extractErrorMessage(error) || t('exchangeRate.deleteFailed'));
...
const handleUpdateFreightRates = async () => {
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || t("exchangeRate.freightUpdateFailed"));
...
const handleUpdateCountryFreightRates = async () => {
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || t("exchangeRate.countryFreightUpdateFailed"));
