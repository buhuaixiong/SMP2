...
import type { QuoteExportData, QuotePriceComparison } from "@/types";
...
async function handleExport(format: "json" | "excel" | "pdf") {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "Failed to export quote");
  } finally {
    exporting.value = false;
  }
}

async function downloadJSON(data: QuoteExportData) {
...
function generateExcel(data: QuoteExportData) {
  try {
    ...
    if (data.priceComparisons && data.priceComparisons.length > 0) {
      ...
      data.priceComparisons.forEach((comp: QuotePriceComparison) => {
        quoteData.push([
    ...
    if (data.review) {
      ...
      quoteData.push(["Selected Quote ID", data.review.selectedQuoteId ?? "-"]);
      quoteData.push(["Quality Score", data.review.reviewScores?.quality || "-"]);
      quoteData.push(["Price Score", data.review.reviewScores?.price || "-"]);
      quoteData.push(["Delivery Score", data.review.reviewScores?.delivery || "-"]);
      quoteData.push(["Service Score", data.review.reviewScores?.service || "-"]);
      quoteData.push(["Comments", data.review.comments || "-"]);
      quoteData.push(["Reviewed By", data.review.reviewedBy || "-"]);
      quoteData.push(["Reviewed At", data.review.reviewedAt || "-"]);
    }
...
function generatePDF(data: QuoteExportData) {
