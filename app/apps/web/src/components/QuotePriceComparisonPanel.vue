...
import { type FormInstance, type FormRules, type UploadFile, type UploadInstance, type UploadUserFile } from "element-plus";
...
async function loadComparisons() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "Failed to load price comparisons");
  } finally {
    loading.value = false;
  }
}
...
function handleFileChange(file: UploadFile) {
  ...
  form.value.screenshot = file.raw ?? null;
}
...
async function submitComparison() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || "Failed to add price comparison");
  } finally {
