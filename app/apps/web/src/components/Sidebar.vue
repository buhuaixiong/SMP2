...
const canViewItemMaster = computed(() =>
  authStore.hasAnyPermission("item_master.view.own", "item_master.view.all"),
);
...
const normalizedRole = computed(() => String(authStore.user?.role ?? ""));
const normalizedAccountType = computed(() => String(authStore.user?.accountType ?? ""));
...
const menuItems = computed<MenuItem[]>(() => {
  const role = normalizedRole.value;
  const accountType = normalizedAccountType.value;
  ...
  if (role === "purchaser") {
    ...
    if (canViewItemMaster.value) {
      items.push({ path: "/item-master", label: "sidebar.itemMaster", icon: Operation });
    }
  ...
  if (role === "admin") {
    ...
    if (canViewItemMaster.value) {
      items.splice(2, 0, { path: "/item-master", label: "sidebar.itemMaster", icon: Operation });
    }
...
onMounted(() => {
  ...
  if (
    (normalizedRole.value === "temp_supplier" || normalizedRole.value === "formal_supplier") &&
    !hasLoaded.value
  ) {
