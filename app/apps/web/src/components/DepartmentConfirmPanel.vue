...
type DepartmentConfirmRfq = Rfq & {
  lineItems?: RfqItem[];
};

type DepartmentConfirmQuote = Quote & {
  supplierName?: string;
  currency?: string;
  totalAmount?: number;
...
const props = defineProps<{
  rfqId: number;
  rfq: DepartmentConfirmRfq | null;
  selectedQuote: DepartmentConfirmQuote | null;
...
async function handleSubmit() {
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || t("rfq.pr.confirmError"));
    }
  } finally {
