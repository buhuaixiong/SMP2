...
import type { Rfq } from '@/types/index'
import { extractErrorMessage } from '@/utils/errorHandling'
import { useNotification } from "@/composables";

const notification = useNotification();

interface InvalidLineItemDetail {
  id: number
...
interface PrGenerationError {
  code?: string
  message?: string
  details?: {
    invalidItems?: InvalidLineItemDetail[]
    missingQuoteItems?: InvalidLineItemDetail[]
  }
}
...
function handleInvalidStatusError(error: PrGenerationError) {
...
function handleMissingQuoteError(error: PrGenerationError) {
  ...
  const itemList = missingItems
    .map((item) => (item.lineNumber ? `#${item.lineNumber}` : `ID:${item.id}`))
    .join(', ')
...
async function handleGenerateAndDownload() {
  try {
    ...
    const normalizedError = (typeof error === 'object' && error !== null ? error : {}) as PrGenerationError

    if (normalizedError.code === 'INVALID_LINE_ITEM_STATUS') {
      handleInvalidStatusError(normalizedError)
    } else if (normalizedError.code === 'MISSING_SELECTED_QUOTE') {
      handleMissingQuoteError(normalizedError)
    } else {
      const errorCode = normalizedError.code || 'UNKNOWN'
      const errorMessage = extractErrorMessage(error) || t('rfq.pr.generateFailed')
    ...
    if (normalizedError.details) {
      console.error('[PR Excel] Error details:', normalizedError.details)
    }
  } finally {
