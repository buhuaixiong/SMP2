...
const loadRegistrationStatus = async () => {
  const supplierId = supplierSelfId.value;
  if (supplierId == null || !shouldCheckRegistrationStatus.value) {
    registrationStatus.value = null;
    return;
  }
  ...
  try {
    registrationStatus.value = await fetchSupplierRegistrationStatusBySupplier(supplierId);
  } catch (error) {
