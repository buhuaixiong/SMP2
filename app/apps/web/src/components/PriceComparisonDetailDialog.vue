...
import { createAuthorizedObjectUrl, revokeObjectUrl } from '@/utils/fileDownload';

interface PriceComparisonDetailData {
...
interface Props {
  visible: boolean;
  detailData: PriceComparisonDetailData | null;
...
const loadImagePreview = async () => {
  ...
  const downloadUrl = props.detailData?.downloadUrl;
  if (!downloadUrl) {
    return;
  }

  try {
    imagePreviewUrl.value = await createAuthorizedObjectUrl(String(downloadUrl));
  } catch (error) {
    console.error('[PriceComparisonDetailDialog] Failed to load preview image', error);
  }
};

watch(
  () => props.detailData?.downloadUrl,
  () => {
