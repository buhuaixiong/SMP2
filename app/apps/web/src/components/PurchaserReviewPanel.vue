...
import type { Quote, Rfq, RfqItem, RfqQuoteItem } from "@/types/index";
...
type ReviewQuote = Quote & {
  supplierName: string;
  lineItems: RfqQuoteItem[];
};
...
type ComparisonRow = {
  ...
  quantity?: number;
  unit?: string;
  quotes: Record<number, RfqQuoteItem>;
};
...
const priceComparisons = reactive<Record<string, PriceComparisonDraft>>({
...
const comparisonData = computed(() => {
  ...
  return props.rfq.lineItems.map((lineItem): ComparisonRow => {
    const quotesMap: Record<number, RfqQuoteItem> = {};

    props.quotes.forEach((quote) => {
      const quoteLineItem = quote.lineItems.find((ql) => ql.rfqLineItemId === lineItem.id);
      if (quoteLineItem) {
...
function selectQuote(quote: ReviewQuote) {
...
function handleUploadSuccess(response: UploadSuccessResponse, platform: string) {
  if (response.data) {
...
async function handleSubmitReview() {
  ...
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || t("rfq.review.submitError"));
    }
  } finally {
