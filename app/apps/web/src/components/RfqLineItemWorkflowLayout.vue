...
const quoteById = computed(() => {
  const map = new Map<number, QuoteLike>()
  quotes.value.forEach((quote) => {
    const quoteId = pickFirstNumber(quote.id)
    if (quoteId !== null) {
      map.set(quoteId, quote)
    }
  })
  return map
})
...
const lineItemSummary = computed(() => {
  let pending = 0
  let completed = 0
  const pendingPo: NormalizedWorkflowLineItem[] = []
  const byId = new Map<number, NormalizedWorkflowLineItem>()
  ...
  lineItems.value.forEach((item) => {
    ...
    if (status === 'pending_director' || status === 'pending_po') {
      pending += 1
    }
  })
  ...
  return {
    ...
    pendingPoItems: pendingPo,
    lineItemById: byId,
  }
})

const quoteLineItemsByQuoteAndLineId = computed(() => {
  const map = new Map<number, Map<number, QuoteItemLike>>()
  quoteById.value.forEach((quote, quoteId) => {
    const quoteLineItems = (quote.lineItems || quote.items || quote.quoteItems || [])
    const lineItemMap = new Map<number, QuoteItemLike>()
    quoteLineItems.forEach((quoteLineItem) => {
      const linkedId = pickFirstNumber(
        firstDefinedValue(
          quoteLineItem.rfqLineItemId,
          quoteLineItem.lineItemId,
          quoteLineItem.rfq_line_item_id
        )
      )
      if (linkedId !== null && !lineItemMap.has(linkedId)) {
        lineItemMap.set(linkedId, quoteLineItem)
      }
    })
    map.set(quoteId, lineItemMap)
  })
  return map
})
...
const selectedLineItem = computed(() => {
  ...
  return lineItemSummary.value.lineItemById.get(selectedLineItemId.value) ?? null
})

const selectedQuote = computed(() => {
  if (!selectedQuoteId.value) return null
  return quoteById.value.get(selectedQuoteId.value) ?? null
})
...
const pendingCount = computed(() => {
  return lineItemSummary.value.pendingCount
})

const completedCount = computed(() => {
  return lineItemSummary.value.completedCount
})

const pendingPoItems = computed(() => {
  return lineItemSummary.value.pendingPoItems
})

const pendingPoItemsWithQuotes = computed(() => {
  return pendingPoItems.value.map((item) => {
    const selectedQuoteId = pickFirstNumber(item.selectedQuoteId, item.selected_quote_id) ?? null
    const selectedQuote = selectedQuoteId !== null
      ? quoteById.value.get(selectedQuoteId) ?? null
      : null
    const matchedQuoteLineItem = selectedQuoteId !== null
      ? quoteLineItemsByQuoteAndLineId.value.get(selectedQuoteId)?.get(item.id) ?? null
      : null
...
function handleSelectLineItem(itemId: number) {
  ...
  const item = lineItemSummary.value.lineItemById.get(itemId)
