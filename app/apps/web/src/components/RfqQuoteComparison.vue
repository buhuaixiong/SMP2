    ...
    <div v-else class="comparison-content">
      ...
      >
        ...
        <el-table-column :label="t('rfq.quotes.taxStatus')" width="110">
          <template #default="{ row }">
            <el-tag :type="getEffectiveTaxStatus(row) === 'inclusive' ? 'success' : 'warning'" size="small">
              {{ t(`rfq.taxStatus.${getEffectiveTaxStatus(row) || "inclusive"}`) }}
            </el-tag>
          </template>
        </el-table-column>
              ...
              >
                ...
                >
                  <el-link
                    ...
                    :underline="false"
                    @click.prevent="openDownloadUrl(record.downloadUrl)"
                  >
      ...
      <div v-if="selectedQuote" class="quote-detail">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item :label="t('rfq.quotes.taxStatus')">
            <el-tag
              :type="getEffectiveTaxStatus(selectedQuote) === 'inclusive' ? 'success' : 'warning'"
              size="small"
            >
              {{ t(`rfq.taxStatus.${getEffectiveTaxStatus(selectedQuote) || "inclusive"}`) }}
            </el-tag>
          </el-descriptions-item>
          ...
          <el-descriptions-item :label="t('rfq.quote.shippingCountry')">
            {{
              selectedQuote.shippingCountry ||
                selectedQuote.shipping_country ||
                "-"
            }}
          </el-descriptions-item>
            ...
            <el-table-column :label="t('rfq.quotes.fileName')" min-width="220">
              <template #default="{ row }">
                <el-link
                  ...
                  type="primary"
                  @click.prevent="openDownloadUrl(row.downloadUrl)"
                >
        ...
        >
          <template #header>
            <el-link
              ...
              @click.stop="handleSupplierNameClickInComparison(quote)"
            >
              {{ quote.companyName || quote.supplierName }}
          ...
          <template #default="{ row }">
            <div v-if="row.key === 'quoteAttachments'" class="attachment-list">
              <template v-if="row.values[quote.id] && row.values[quote.id].length">
                <div
                  v-for="file in row.values[quote.id]"
                  :key="file.id || file.storedName || file.originalName || file.downloadUrl"
                >
                  <el-link
                    v-if="file.downloadUrl"
                    :underline="false"
                    @click.prevent="openDownloadUrl(file.downloadUrl)"
                  >
                    {{ file.originalName || file.storedName || '-' }}
                  </el-link>
                  <span v-else>{{ file.originalName || file.storedName || '-' }}</span>
                </div>
              </template>
              <span v-else>-</span>
            </div>
            <div v-else :class="{ 'best-value': row.bestQuoteId === quote.id }">
              {{ row.values[quote.id] }}
            </div>
          </template>
        </el-table-column>
...
import { summarizeNumericValues } from "@/utils/rfqPriceComparisonIndex";
...
type QuoteItemLike = RfqQuoteItem & {
  ...
  rfqQuantity?: number | string | null;
  lineQuantity?: number | string | null;
  tariffLineTotalUsd?: number | string | null;
  tariffLineTotal?: number | string | null;
...
type RawPriceComparisonRecord = PriceComparisonRecord & {
  ...
  platformPrice?: number | string | null;
...
type DetailValue = string | QuoteAttachment[] | null;
...
interface Props {
  ...
  existingPriceComparisons?: RawPriceComparisonRecord[];
}
...
const selectedQuote = ref<QuoteLike | null>(null);
const showSupplierProfileDialog = ref(false);
const selectedSupplierQuote = ref<QuoteLike | null>(null);
...
const selectedQuoteAttachments = computed<QuoteAttachment[]>(() => {
  return normalizeQuoteAttachments(selectedQuote.value?.attachments ?? []);
});

const openDownloadUrl = async (downloadUrl?: string | null) => {
  if (!downloadUrl) {
    return;
  }
...
const normalizeComparisonRecord = (record: RawPriceComparisonRecord): PriceComparisonRecord => {
...
const populateExistingComparisons = (records: RawPriceComparisonRecord[] | undefined | null) => {
...
const getQuoteUnitPrice = (quote: QuoteLike): number | null => {
  // Try direct unit price first
  let priceValue = quote.unitPrice ?? quote.unit_price ?? null;
  ...
  if (!priceValue && quote.quoteItems && Array.isArray(quote.quoteItems) && quote.quoteItems.length > 0) {
    const validPrices = quote.quoteItems
      .map((item) => toNumeric(item.unitPrice ?? item.unit_price))
...
const getQuoteTariffSummary = (quote: QuoteLike): QuoteTariffSummary | null => {
  const summary =
...
const getQuoteTotalQuantity = (quote: QuoteLike): number => {
  if (!quote.quoteItems || quote.quoteItems.length === 0) {
    return 0;
  }
  return quote.quoteItems.reduce((sum, item: QuoteItemLike) => {
    const quantity =
      Number(item.quantity ?? item.rfqQuantity ?? item.lineQuantity ?? item.minimumOrderQuantity ?? 0);
...
const quoteHasSpecialTariff = (quote: QuoteLike): boolean => {
  const summary = getQuoteTariffSummary(quote);
...
const unitPriceSummary = computed(() =>
  summarizeNumericValues(props.quotes.map((quote) => getQuoteUnitPrice(quote)))
);

const lowestPrice = computed(() => {
  return unitPriceSummary.value.min;
});

const highestPrice = computed(() => {
  return unitPriceSummary.value.max;
});

const averagePrice = computed(() => {
  return unitPriceSummary.value.avg;
});
...
const standardCostSummary = computed(() =>
  summarizeNumericValues(
    Object.values(standardCostInfoMap.value).map((info) => {
...
const lowestStandardCost = computed(() => {
  return standardCostSummary.value.min;
});

const highestStandardCost = computed(() => {
  return standardCostSummary.value.max;
});

const averageStandardCost = computed(() => {
  return standardCostSummary.value.avg;
});
...
const getRowClassName = ({ row }: { row: QuoteLike }): string => {
  ...
  const rowUnitPrice = toNumeric(row.unitPrice ?? row.unit_price);
...
const handleRowClick = (row: QuoteLike) => {
  viewDetails(row);
};

const viewDetails = (quote: QuoteLike) => {
  selectedQuote.value = quote;
  showDetailDialog.value = true;
};

const viewSupplierProfile = (quote: QuoteLike) => {
...
// Handle supplier name click in comparison dialog
const handleSupplierNameClickInComparison = (quote: QuoteLike) => {
  selectedSupplierQuote.value = quote;
  showSupplierProfileDialog.value = true;
};
...
const detailComparisonData = computed(() => {
  ...
  const fields = [
    ...
    { key: 'quoteAttachments', label: 'rfq.quoteComparison.quoteAttachments' },
  ];

  return fields.map(field => {
    const values: Record<number, DetailValue> = {};
    let bestQuoteId: number | null = null;
    let bestValue: number | null = null;

    props.quotes.forEach(quote => {
      const info = standardCostInfoMap.value[quote.id];
      let value: DetailValue = null;
      ...
      switch (field.key) {
        ...
        case 'productOrigin':
          ...
          break;
        case 'quoteAttachments': {
          const attachments = normalizeQuoteAttachments(quote.attachments ?? []);
...
const getQuoteUnit = (quote: QuoteLike): string => {
  if (quote.quoteItems && quote.quoteItems.length > 0) {
...
const getEffectiveTaxStatus = (row: QuoteLike): string => {
  // For quote level (row is the quote itself)
  if (row.taxStatus) return row.taxStatus;
  // For item level (check quoteItems)
  if (row.quoteItems && row.quoteItems.length > 0 && row.quoteItems[0]?.taxStatus) {
    return row.quoteItems[0].taxStatus;
  }
  return 'inclusive';
};
