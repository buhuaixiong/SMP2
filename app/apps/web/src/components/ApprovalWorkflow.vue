      ...
      <el-timeline>
        ...
        >
          <div class="approval-step">
            ...
            <!-- 评论区 -->
            <div v-if="getApprovalComments(approval).length > 0" class="comments-section">
              <h5>{{ t("rfq.approval.comments") }}</h5>
              <div
                v-for="(comment, index) in getApprovalComments(approval)"
...
import type { RfqApprovalComment, RfqApprovalStep } from "@/api/rfq";
import type { Quote, Rfq, User } from "@/types/index";
...
type ApprovalStep = RfqApprovalStep & {
  comments?: RfqApprovalComment[] | string;
  decision?: string;
};

type ApprovalQuote = Quote & {
  lineItems?: Array<{ rfqLineItemId?: number; unitPrice?: number | null }>;
};

type ApprovalPriceComparison = {
  id: number;
  platform: string;
...
const props = defineProps<{
  rfqId: number;
  rfq: Rfq;
  selectedQuote: ApprovalQuote | null;
  approvals: ApprovalStep[];
  priceComparisons: ApprovalPriceComparison[];
}>();
...
function canApprove(approval: ApprovalStep): boolean {
  if (approval.status !== "pending") return false;
  return approval.approverRole === authStore.user?.role;
}

function canInvitePurchaser(approval: ApprovalStep): boolean {
  if (approval.status !== "pending") return false;
...
function getQuotePrice(lineItemId: number): string {
  ...
  const quoteLineItem = props.selectedQuote.lineItems.find(
    (item) => item.rfqLineItemId === lineItemId,
  );
...
function getApprovalComments(approval: ApprovalStep): RfqApprovalComment[] {
  return Array.isArray(approval.comments) ? approval.comments : [];
}
...
async function handleApprove(approvalId: number) {
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || t("rfq.approval.approveError"));
    }
  }
}
...
async function handleReject(approvalId: number) {
  try {
    ...
    if (error !== "cancel") {
      notification.error(extractErrorMessage(error) || t("rfq.approval.rejectError"));
    }
  }
}
...
async function handleAddComment(approvalId: number) {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("rfq.approval.commentError"));
  }
}
...
async function handleInvitePurchasers() {
  ...
  try {
    ...
    notification.error(extractErrorMessage(error) || t("rfq.approval.inviteError"));
  }
}
