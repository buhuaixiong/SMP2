          ...
          <el-table-column :label="t('rfq.quote.moq')" width="150">
            ...
            <template #default="{ row, $index }">
              <el-form-item
                ...
                :rules="[
                  {
                    validator: (_rule: FormItemRule, value: unknown, callback: ValidatorCallback) =>
                      validateRequiredWhenPriced(
                        row.rfqLineItemId,
                        value,
                        callback,
                        t('rfq.quote.moqRequiredWhenPriced'),
                        (candidate: unknown) => typeof candidate !== 'number' || candidate <= 0,
                      ),
                    trigger: 'change'
                  }
                ]"
                ...
                :rules="[
                  {
                    validator: (_rule: FormItemRule, value: unknown, callback: ValidatorCallback) =>
                      validateRequiredWhenPriced(
                        ...
                        value,
                        callback,
                        t('rfq.quote.spqRequiredWhenPriced'),
                        (candidate: unknown) => typeof candidate !== 'number' || candidate <= 0,
                      ),
                    trigger: 'change'
                  }
                  ...
                  <div class="tariff-cell__meta">
                    <span>
                      {{ t("rfq.quote.tariffRateLabel", {
                        rate: formatPercentage(getTariffRate(row.rfqLineItemId)),
                      }) }}
                    </span>
                ...
                :rules="[
                  {
                    validator: (_rule: FormItemRule, value: unknown, callback: ValidatorCallback) =>
                      validateRequiredWhenPriced(
                        ...
                        value,
                        callback,
                        t('rfq.quote.productOriginRequiredWhenPriced'),
...
import type { FormInstance, FormItemRule, UploadFile, UploadInstance, UploadUserFile } from "element-plus";
import { Loading, QuestionFilled } from "@element-plus/icons-vue";
import { BASE_URL } from "@/api/http";
import type { Quote, QuoteTariffCalculation, Rfq } from "@/types/index";
import {
  fetchTariffOptions,
  calculateTariff,
...
interface QuoteLineItem extends RfqLineItem {
  ...
  tariffCalculation?: TariffCalculationResult | QuoteTariffCalculation | null;
}

type ValidatorCallback = (error?: Error) => void;

interface RawRfqItem extends Record<string, unknown> {
  id?: number;
  materialCategory?: string;
  materialType?: string;
...
interface ExistingQuoteItem {
  ...
  tariffCalculation?: TariffCalculationResult | QuoteTariffCalculation | null;
  tariff_calculation?: TariffCalculationResult | QuoteTariffCalculation | null;
...
type ExistingQuote = Quote & {
  quoteItems?: ExistingQuoteItem[];
  shipping_country?: string | null;
};

const props = defineProps<{
  rfq: SupplierQuoteRfq;
...
const lineTariffs = reactive<Record<number, TariffCalculationResult | QuoteTariffCalculation | null>>({});
...
const validateRequiredWhenPriced = (
  rfqLineItemId: number,
  value: unknown,
  callback: ValidatorCallback,
  message: string,
  isEmpty: (candidate: unknown) => boolean,
) => {
  const hasPrice = formData.lineItems[rfqLineItemId]?.unitPrice !== undefined &&
    formData.lineItems[rfqLineItemId]?.unitPrice !== null;

  if (hasPrice && isEmpty(value)) {
...
const numberOrUndefined = (...values: unknown[]) => {
  const parsed = pickFirstNumber(...values);
  return parsed === null ? undefined : parsed;
};

const stringOrFallback = (fallback: string, ...values: Array<string | null | undefined>) => {
  const parsed = firstDefinedValue(...values);
...
function initializeQuoteLineItems() {
  if (props.rfq?.items && props.rfq.items.length > 0) {
    const rawItems = props.rfq.items as RawRfqItem[];
    const existingQuoteItems: ExistingQuoteItem[] = props.existingQuote?.quoteItems ?? [];

    quoteLineItems.value = rawItems.map((rawItem) => {
      const normalizedItem: RfqLineItem = {
        ...rawItem,
        id: Number(rawItem.id ?? 0),
        rfqLineItemId: Number(rawItem.id ?? 0),
        ...
        specifications: stringOrFallback(
          '',
          rawItem.specifications,
          rawItem.parameters as string | null | undefined,
          rawItem.remarks as string | null | undefined,
        ),
        quantity: numberOrUndefined(rawItem.quantity, 0) ?? 0,
        unit: stringOrFallback('', rawItem.unit as string | null | undefined),
        brand: stringOrFallback('', rawItem.brand as string | null | undefined),
        currency: rawItem.currency ?? props.rfq?.currency ?? "CNY",
      };

      const existingItem = existingQuoteItems.find((qi) => qi.rfqLineItemId === normalizedItem.id);

      const unitPrice = numberOrUndefined(existingItem?.unitPrice, existingItem?.unit_price);
      const quantity = Number(normalizedItem.quantity ?? 0);
      const totalPrice = Number.isFinite(quantity) && unitPrice !== undefined && unitPrice !== null
        ? unitPrice * quantity
        : numberOrUndefined(existingItem?.totalPrice, existingItem?.total_price, 0) ?? 0;

      formData.lineItems[normalizedItem.id] = {
        unitPrice: unitPrice,
        minimumOrderQuantity: numberOrUndefined(
          existingItem?.minimumOrderQuantity,
          existingItem?.moq,
          normalizedItem.quantity,
          1
        ),
        standardPackageQuantity: numberOrUndefined(
          existingItem?.standardPackageQuantity,
          undefined
        ),
        deliveryPeriod: numberOrUndefined(existingItem?.deliveryPeriod, undefined),
        productOrigin: stringOrFallback('', existingItem?.productOrigin, existingItem?.product_origin),
        productGroup: "OTHERS",
      };
      ...
      return {
        ...
        notes: stringOrFallback('', existingItem?.notes),
        brand: stringOrFallback('', existingItem?.brand, normalizedItem.brand),
        productOrigin: formData.lineItems[normalizedItem.id].productOrigin,
        productGroup: "OTHERS",
        standardUnitCostLocal: numberOrUndefined(
          existingItem?.standardLineCostLocal,
          existingItem?.standardUnitCostLocal,
          null
        ),
        standardUnitCostUsd: numberOrUndefined(
    ...
    if (props.existingQuote) {
      formData.notes = stringOrFallback('', props.existingQuote.notes);
      formData.deliveryTerms = stringOrFallback('', props.existingQuote.deliveryTerms);
      formData.shippingCountry = stringOrFallback(
        '',
        props.existingQuote.shippingCountry,
        props.existingQuote.shipping_country,
      );
      formData.taxStatus = stringOrFallback("inclusive", props.existingQuote.taxStatus);
    }
    formData.currency = stringOrFallback(
      formData.currency,
...
function getTariffResult(
  rfqLineItemId: number,
): TariffCalculationResult | QuoteTariffCalculation | null {
  return lineTariffs[rfqLineItemId] || null;
}

function getTariffRate(rfqLineItemId: number): number | null {
  const tariff = getTariffResult(rfqLineItemId);
  if (!tariff) {
    return null;
  }
  if ("freightRate" in tariff) {
    return tariff.freightRate ?? null;
  }
  return tariff.tariffRate ?? null;
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const url = buildRfqQuoteSubmissionUrl(BASE_URL, props.rfq.id, props.existingQuote?.id);
    ...
    if (!response.ok) {
      const errorMessage = await parseRfqQuoteSubmissionError(response);
    ...
    submissionFeedback.value = {
      type: "error",
      message: extractErrorMessage(error) || String(t("rfq.quote.submitError")),
    };
  } finally {
 ...
 * 妫€鏌ユ煇琛屾槸鍚﹀凡濉啓浠锋牸
 */
function hasPrice(row: QuoteLineItem): boolean {
 ...
 * 妫€鏌ユ煇琛屾槸鍚﹀凡濉啓澶囨敞
 */
function hasNotes(row: QuoteLineItem): boolean {
  return Boolean(row.notes && typeof row.notes === 'string' && row.notes.trim().length > 0);
}

/**
 * 鍒ゆ柇澶囨敞鏄惁涓哄繀濉紙鏃犱环鏍兼椂蹇呭～锛?
 */
function isNotesRequired(row: QuoteLineItem): boolean {
 ...
 * 鍔ㄦ€佸崰浣嶇鏂囨湰
 */
function getNotesPlaceholder(row: QuoteLineItem): string {
  if (hasPrice(row)) {
    return t('rfq.lineItems.notesPlaceholder'); // "鍙€夊娉?
  } else {
    return t('rfq.lineItems.notesRequiredPlaceholder'); // "璇疯鏄庢棤娉曟姤浠风殑鍘熷洜锛堝繀濉級"
  }
}
 ...
 * 蹇€熷～鍏?鏃犳硶鎶ヤ环"妯℃澘
 */
function fillNoQuoteTemplate(row: QuoteLineItem): void {
  row.notes = t('rfq.quote.noQuoteTemplate');
  // 妯℃澘鍐呭: "璇ョ墿鏂欐殏鏃犺揣婧?涓嶅湪鎴戝徃渚涘簲鑼冨洿鍐?
