...
import { downloadFile, openFileInNewTab, resolveUploadDownloadUrl } from '@/utils/fileDownload'
import type {
  Quote,
  QuoteAttachment,
...
async function openAttachment(attachment: QuoteAttachment) {
  if (!attachment.downloadUrl) {
    return
  }
  try {
    await openFileInNewTab(String(attachment.downloadUrl))
  } catch (error) {
    console.error('[RfqPriceComparisonSection] open attachment failed', error)
  }
}
