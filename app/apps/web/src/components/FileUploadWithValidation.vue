...
interface Emits {
  ...
  (e: "success", response: UploadSuccessResponse, file: UploadFile): void;
  (e: "error", error: Error | string, file: UploadFile): void;
  ...
  (newValue) => {
    if (newValue && newValue.length > 0) {
      fileList.value = newValue.map((file) => ({
        name: file.name || file.original_name || "uploaded-file",
        url: file.url || file.file_path || undefined,
        status: "success",
...
function handleSuccess(response: UploadSuccessResponse, file: UploadFile) {
...
function handleError(error: Error | string, file: UploadFile) {
  // Remove from scanning list
  const index = scanningFiles.value.findIndex((f) => f.name === file.name);
  if (index !== -1) {
    scanningFiles.value.splice(index, 1);
...
defineExpose({
  ...
  abort: (file: UploadFile | UploadRawFile) => uploadRef.value?.abort(file as UploadFile),
});
