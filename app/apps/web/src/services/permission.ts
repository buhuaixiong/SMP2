...
export const permissionService: ServiceDefinition<PermissionService> = {
  ...
  dependencies: ["http"],
    setup() {
      ...
      let role: string | null = authStore.user?.role ?? null;
      let lastSyncedAt = 0;
    ...
    const ensureFresh = async (force?: boolean) => {
      ...
      const response = await apiFetch<{
        user?: PermissionUserPayload;
        data?: { user?: PermissionUserPayload };
      }>("/auth/me");
