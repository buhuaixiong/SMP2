...
export async function fetchRfqComparisonPrintData(
  id: number,
  scope: RfqComparisonPrintScope = "latest",
): Promise<RfqComparisonPrintData> {
  const response = await apiFetch<{ data: RfqComparisonPrintData }>(
    `/rfq-workflow/${id}/comparison-print-data`,
    { params: { scope } },
  );
  return response.data;
}

export async function fetchRfqBidRounds(id: number): Promise<RfqBidRoundSummary[]> {
  const response = await apiFetch<{ data: RfqBidRoundSummary[] }>(`/rfq-workflow/${id}/bid-rounds`);
  return response.data ?? [];
}

export async function fetchRfqSupplierComparisonHistory(
  id: number,
): Promise<RfqSupplierComparisonHistory> {
  const response = await apiFetch<{ data: RfqSupplierComparisonHistory }>(
    `/rfq-workflow/${id}/supplier-comparison-history`,
  );
  return response.data;
}

export async function extendRfqBidDeadline(
  id: number,
  payload: ExtendRfqBidDeadlinePayload,
): Promise<RfqBidRoundSummary> {
  const response = await apiFetch<{ data: RfqBidRoundSummary }>(`/rfq-workflow/${id}/extend-bid-deadline`, {
    method: "POST",
    body: payload,
  });
  return response.data;
...
export async function startNextRfqBidRound(
  id: number,
  payload: StartNextRfqBidRoundPayload,
): Promise<RfqBidRoundSummary> {
  const response = await apiFetch<{ data: RfqBidRoundSummary }>(`/rfq-workflow/${id}/start-next-round`, {
    method: "POST",
    body: payload,
  });
  return response.data;
...
export async function updateRfqViewLink(rfqId: number, viewLink: string): Promise<string> {
  const response = await apiFetch<{ data?: { viewLink?: string | null } }>(`/rfq-workflow/${rfqId}/view-link`, {
    method: "PUT",
    body: { viewLink },
  });
  return response.data?.viewLink ?? viewLink;
}
