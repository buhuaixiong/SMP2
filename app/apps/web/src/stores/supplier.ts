...
export const useSupplierStore = defineStore("supplier", () => {
  ...
  const inFlightSupplierRequests = reactive(new Map<string, Promise<Supplier[]>>());
  ...
  const fetchSuppliers = async (
    ...
    const targetPage =
      ...
      normalizedOptions.page > 0
        ? Math.floor(normalizedOptions.page)
        : currentPage.value;
    ...
    const targetPageSize =
      ...
      normalizedOptions.pageSize > 0
        ? Math.floor(normalizedOptions.pageSize)
        : pageSize.value;

    const fetchKey = supplierPortalUser
      ? `portal|supplier:${authStore.user?.supplierId ?? "none"}|force:${normalizedOptions.force ? 1 : 0}`
      : `directory|filters:${stableSerialize(cloneDirectoryFilters(filters))}|page:${targetPage}|pageSize:${targetPageSize}|force:${normalizedOptions.force ? 1 : 0}`;

    if (!normalizedOptions.force) {
      const inFlight = inFlightSupplierRequests.get(fetchKey);
      if (inFlight) {
        return inFlight;
      }
    }
    ...
    if (!normalizedOptions.force) {
      inFlightSupplierRequests.set(fetchKey, request);
    }
    ...
    try {
      ...
      if (!normalizedOptions.force) {
        inFlightSupplierRequests.delete(fetchKey);
      }
    }
  };
