...
public sealed class FilesController : ControllerBase
{
    ...
    private readonly FileAccessTokenStore _fileAccessTokenStore;
    ...
    public FilesController(
        ...
        TempSupplierUpgradeRepository upgradeRepository,
        TempSupplierUpgradeService upgradeService,
        SupplierFileRepository fileRepository,
        FileAccessTokenStore fileAccessTokenStore,
        ...
        ILogger<FilesController> logger)
    {
        ...
        _fileAccessTokenStore = fileAccessTokenStore;
    ...
    public async Task<IActionResult> GenerateDownloadToken([FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        await _fileAccessTokenStore.InsertAsync(
            tokenRecord.Token,
            tokenRecord.UserId,
            tokenRecord.ResourceType,
            tokenRecord.ResourceId,
            tokenRecord.Category,
            tokenRecord.StoragePath,
            tokenRecord.OriginalName,
            tokenRecord.FileSize,
            tokenRecord.MimeType,
        ...
        {
            token,
            downloadEndpoint = "/api/files/secure-download",
            request = new
            {
                token,
                type = resourceType,
                id = fileId.Value.ToString()
            },
    ...
    public async Task<IActionResult> SecureDownload(
        ...
        CancellationToken cancellationToken)
    {
        if (HttpContext.GetAuthUser() == null)
        {
            return Unauthorized(new { message = "Authentication required." });
        }

        var token = ReadString(body, "token");
        var type = ReadString(body, "type") ?? ReadString(body, "resourceType");
        var id = ReadString(body, "id") ?? ReadString(body, "resourceId");
        ...
        var ipAddress = HttpContext.Connection.RemoteIpAddress?.ToString();
        var record = await _fileAccessTokenStore.ConsumeAsync(token, type, id, ipAddress, cancellationToken);
        if (record == null)
        {
