...
{
    private readonly BuyerAssignmentDataService _buyerAssignmentDataService;
    private readonly IAuditService _auditService;
    private readonly ILogger<BuyerAssignmentsController> _logger;

    public BuyerAssignmentsController(
        BuyerAssignmentDataService buyerAssignmentDataService,
        IAuditService auditService,
        IWebHostEnvironment environment,
        ILogger<BuyerAssignmentsController> logger) : base(environment)
    {
        _buyerAssignmentDataService = buyerAssignmentDataService;
    ...
    public async Task<IActionResult> AssignSuppliersByTag([FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var buyer = await _buyerAssignmentDataService.QueryUsers()
        ...
        var suppliers = await (from tag in _buyerAssignmentDataService.QuerySupplierTags()
                               join supplier in _buyerAssignmentDataService.QuerySuppliers()
        ...
        await using var transaction = await _buyerAssignmentDataService.BeginTransactionAsync(cancellationToken);
        try
        {
            foreach (var supplierId in supplierIds)
            {
                var exists = await _buyerAssignmentDataService.QueryAssignments()
                ...
                _buyerAssignmentDataService.AddAssignment(new BuyerSupplierAssignment
                {
                    BuyerId = buyerId,
            ...
            await _buyerAssignmentDataService.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IActionResult> GetAssignedSuppliers([FromQuery] string? buyerId, CancellationToken cancellationToken)
    {
        ...
        var assignments = await (from assignment in _buyerAssignmentDataService.QueryAssignments()
                                 join supplier in _buyerAssignmentDataService.QuerySuppliers()
    ...
    public async Task<IActionResult> DeleteAssignment(int id, CancellationToken cancellationToken)
    {
        ...
        var assignment = await _buyerAssignmentDataService.QueryAssignments(asNoTracking: false)
        ...
        _buyerAssignmentDataService.RemoveAssignment(assignment);
        await _buyerAssignmentDataService.SaveChangesAsync(cancellationToken);
    ...
    {
        var buyers = await _buyerAssignmentDataService.QueryUsers()
