...
public sealed partial class ReconciliationController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var reconciliation = await _reconciliationStore.FindReconciliationForUpdateAsync(reconciliationId, cancellationToken);
        ...
        var duplicateInvoice = await _reconciliationStore.HasDuplicateInvoiceNumberAsync(reconciliation.SupplierId, invoiceNumber, cancellationToken);
        if (duplicateInvoice)
        {
        ...
        try
        {
            ...
            await using var transaction = await _reconciliationStore.BeginTransactionAsync(cancellationToken);
            ...
            _reconciliationStore.AddInvoice(invoice);
            await _reconciliationStore.SaveChangesAsync(cancellationToken);

            _reconciliationStore.AddInvoiceFile(new InvoiceFile
            {
            ...
            var receipt = reconciliation.WarehouseReceiptId.HasValue
                ? await _reconciliationStore.FindWarehouseReceiptAsync(reconciliation.WarehouseReceiptId.Value, cancellationToken)
                : null;
            ...
            {
                _reconciliationStore.AddVarianceAnalysis(new ReconciliationVarianceAnalysis
                {
                    ReconciliationId = reconciliation.Id,
            ...
            var existingMatch = await _reconciliationStore.FindMatchForUpdateAsync(reconciliation.Id, cancellationToken);

            if (existingMatch == null)
            {
                _reconciliationStore.AddMatch(new InvoiceReconciliationMatch
                {
                    ReconciliationId = reconciliation.Id,
                    InvoiceId = invoice.Id,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var reconciliation = await _reconciliationStore.FindReconciliationForUpdateAsync(request.ReconciliationId, cancellationToken);
        if (reconciliation == null)
        {
            return NotFound("Reconciliation not found.");
        }

        var invoice = await _reconciliationStore.FindInvoiceForUpdateAsync(request.InvoiceId, cancellationToken);
        if (invoice == null)
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var reconciliation = await _reconciliationStore.FindReconciliationForUpdateAsync(id, cancellationToken);
        if (reconciliation == null)
        {
            return NotFound("Reconciliation not found.");
        }
        ...
        try
        {
            await _stateMachine.TransitionAsync(
                ...
                async (entity, status, ct) =>
                {
                    ...
                    await _reconciliationStore.SaveChangesAsync(ct);
                    return entity;
                },
                cancellationToken);
        }
    ...
    public async Task<IActionResult> DeleteReconciliation(int id, CancellationToken cancellationToken)
    {
        ...
        var matches = await _reconciliationStore.LoadMatchesForDeleteAsync(id, cancellationToken);
        var analyses = await _reconciliationStore.LoadAnalysesForDeleteAsync(id, cancellationToken);
        var histories = await _reconciliationStore.LoadHistoriesForDeleteAsync(id, cancellationToken);
        var invoiceFiles = await _reconciliationStore.LoadInvoiceFilesForDeleteAsync(id, cancellationToken);

        _reconciliationStore.RemoveMatches(matches);
        _reconciliationStore.RemoveAnalyses(analyses);
        _reconciliationStore.RemoveHistories(histories);
        _reconciliationStore.RemoveInvoiceFiles(invoiceFiles);
        _reconciliationStore.RemoveReconciliation(reconciliation);
    ...
    private void AddStatusHistory(
        ...
        string? reason)
    {
        _reconciliationStore.AddStatusHistory(new ReconciliationStatusHistory
        {
