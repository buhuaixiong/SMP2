...
public sealed partial class RequisitionsController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await using var transaction = await _requisitionStore.BeginTransactionAsync(cancellationToken);
        try
        {
            _requisitionStore.AddRequisition(requisition);
            await _requisitionStore.SaveChangesAsync(cancellationToken);

            foreach (var item in items)
            {
                _requisitionStore.AddItem(new MaterialRequisitionItem
                {
            ...
            foreach (var file in form.Attachments)
            {
                ...
                _requisitionStore.AddAttachment(new MaterialRequisitionAttachment
                {
                    RequisitionId = requisition.Id,
    ...
    public async Task<IActionResult> ListRequisitions(CancellationToken cancellationToken)
    {
        ...
        var query = _requisitionStore.QueryRequisitions().AsQueryable();
    ...
    public async Task<IActionResult> GetRequisition(int id, CancellationToken cancellationToken)
    {
        ...
        var requisition = await _requisitionStore.QueryRequisitions()
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requisition = await _requisitionStore.QueryRequisitions(asNoTracking: false)
            .FirstOrDefaultAsync(r => r.Id == id, cancellationToken);
        ...
        try
        {
            ...
            _requisitionStore.UpdateRequisition(requisition);
            await _requisitionStore.SaveChangesAsync(cancellationToken);

            var existingItems = _requisitionStore.QueryItems(asNoTracking: false).Where(i => i.RequisitionId == id);
            _requisitionStore.RemoveItems(existingItems);
            await _requisitionStore.SaveChangesAsync(cancellationToken);
    ...
    private async Task<object?> LoadRequisitionDetailsAsync(int requisitionId, CancellationToken cancellationToken)
    {
        ...
        var items = await _requisitionStore.QueryItems()
            .Where(i => i.RequisitionId == requisitionId)
            .OrderBy(i => i.Id)
            .ToListAsync(cancellationToken);

        var attachments = await _requisitionStore.QueryAttachments()
