...
public sealed partial class ReconciliationController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var supplierMap = await _reconciliationStore.LoadSupplierNameMapAsync(supplierIds, cancellationToken);
        ...
        var receipts = await _reconciliationStore.LoadWarehouseReceiptsAsync(receiptIds, cancellationToken);
        var receiptMap = receipts.ToDictionary(r => r.Id);

        var receiptQuantities = await _reconciliationStore.LoadReceiptQuantityInfosAsync(receiptIds, cancellationToken);
        ...
        var matches = await _reconciliationStore.LoadMatchesForReconciliationsAsync(reconciliationIds, cancellationToken);
        var matchMap = matches
            .GroupBy(m => m.ReconciliationId)
            .ToDictionary(g => g.Key, g => g.First());

        var invoiceIds = matches.Select(m => m.InvoiceId).Distinct().ToList();
        var invoices = await _reconciliationStore.LoadInvoicesAsync(invoiceIds, cancellationToken);
    ...
    {
        var threshold = await _reconciliationStore.LoadLatestThresholdAsync(cancellationToken);
