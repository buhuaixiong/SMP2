...
{
    private readonly ExchangeRateHistoryService _exchangeRateHistoryService;
    ...
    public async Task<IActionResult> GetHistory(CancellationToken cancellationToken)
    {
        ...
        var history = await _exchangeRateHistoryService.GetHistoryAsync(currency, limit, cancellationToken);
    ...
    {
        var currencies = await _exchangeRateHistoryService.GetCurrenciesAsync(cancellationToken);
    ...
    public async Task<IActionResult> UpdateRates([FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var records = new List<ExchangeRateHistory>();
        foreach (var entry in rates)
        {
            records.Add(new ExchangeRateHistory
            {
                Currency = entry.Key,
                Rate = entry.Value,
        ...
        var insertedIds = await _exchangeRateHistoryService.CreateRatesAsync(records, cancellationToken);
    ...
    public async Task<IActionResult> DeleteRate(int id, CancellationToken cancellationToken)
    {
        ...
        var record = await _exchangeRateHistoryService.FindByIdAsync(id, cancellationToken);
        if (record == null)
        {
            return NotFound(new { message = "Exchange rate record not found." });
        }

        await _exchangeRateHistoryService.DeleteAsync(record, cancellationToken);
