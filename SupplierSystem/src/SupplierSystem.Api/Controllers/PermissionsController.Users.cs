...
public sealed partial class PermissionsController
{
    ...
    public async Task<IActionResult> GetAllUsers([FromQuery] string? scope, CancellationToken cancellationToken)
    {
        ...
        var query = _permissionsDataService.QueryUsers();
    ...
    public async Task<IActionResult> GetUserById(string userId, CancellationToken cancellationToken)
    {
        ...
        var user = await _permissionsDataService.QueryUsers()
    ...
    public async Task<IActionResult> CreateUser([FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var existingUser = await _permissionsDataService.FindUserWithUpdateLockAsync(userId, username.Trim(), cancellationToken);
            ...
            {
                var existingEmail = await _permissionsDataService.FindUserByEmailWithUpdateLockAsync(email.Trim(), cancellationToken);
            ...
            _permissionsDataService.AddUser(user);
    ...
    public async Task<IActionResult> UpdateUser(string userId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var user = await _permissionsDataService.QueryUsers(asNoTracking: false).FirstOrDefaultAsync(u => u.Id == userId, cancellationToken);
        if (user == null)
        {
            return NotFound(new { message = "User not found." });
        }
        ...
        {
            var exists = await _permissionsDataService.QueryUsers().AnyAsync(u => u.Email == email && u.Id != userId, cancellationToken);
            if (exists)
            {
                return BadRequest(new { message = $"Email {email} already exists." });
            }
    ...
    public async Task<IActionResult> DeleteUser(string userId, [FromQuery] bool hard, CancellationToken cancellationToken)
    {
        ...
        if (shouldHardDelete)
        {
            var sessions = await _permissionsDataService.QueryActiveSessions(asNoTracking: false)
                .Where(s => s.UserId == user.Id)
                .ToListAsync(cancellationToken);
            var blacklistEntries = await _permissionsDataService.QueryTokenBlacklist(asNoTracking: false)
                .Where(entry => entry.UserId == user.Id)
                .ToListAsync(cancellationToken);

            if (sessions.Count > 0)
            {
                _permissionsDataService.RemoveActiveSessions(sessions);
            }

            if (blacklistEntries.Count > 0)
            {
                _permissionsDataService.RemoveTokenBlacklistEntries(blacklistEntries);
            }

            _permissionsDataService.RemoveUser(user);
            await _permissionsDataService.SaveChangesAsync(cancellationToken);
