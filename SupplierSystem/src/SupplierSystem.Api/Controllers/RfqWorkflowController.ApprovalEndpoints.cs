...
public sealed partial class RfqWorkflowController
{
    ...
    public async Task<IActionResult> GetApprovals(int id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var approvals = await _rfqWorkflowStore.QueryApprovals()
                .Where(a => a.RfqId == id)
            ...
            {
                var comments = await _rfqWorkflowStore.QueryApprovalComments()
    ...
    public async Task<IActionResult> Approve(int id, int approvalId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var approval = await _rfqWorkflowStore.FindApprovalAsync(approvalId, id, asNoTracking: false, cancellationToken);
            if (approval == null)
            {
            ...
            var nextApproval = await _rfqWorkflowStore.FindNextPendingApprovalAsync(id, approval.StepOrder, cancellationToken);
    ...
    public async Task<IActionResult> AddApprovalComment(int id, int approvalId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _rfqWorkflowStore.AddApprovalComment(comment);
            await _rfqWorkflowStore.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IActionResult> InvitePurchasers(int id, int approvalId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var approval = await _rfqWorkflowStore.FindApprovalAsync(approvalId, id, asNoTracking: true, cancellationToken);
            if (approval == null)
            {
                return NotFound(new { message = "Approval not found." });
            }
            ...
            {
                _rfqWorkflowStore.AddApprovalComment(new ApprovalComment
                {
                    ApprovalId = approvalId,
                    AuthorId = user.Id,
                    AuthorName = user.Name,
