...
public sealed class InvoicesController : ApiControllerBase
{
    ...
    {
        var query = _invoiceStore.QueryInvoices();
    ...
    {
        var invoice = await _invoiceStore.GetInvoiceWithSupplierAsync(id, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // 验证供应商存在
        var supplier = await _invoiceStore.FindSupplierAsync(supplier_id, cancellationToken);
        if (supplier == null)
        ...
        _invoiceStore.AddInvoice(invoice);
        await _invoiceStore.SaveChangesAsync(cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var invoice = await _invoiceStore.FindInvoiceAsync(id, cancellationToken);
        if (invoice == null)
            return NotFound("Invoice not found.");
    ...
    public async Task<IActionResult> DeleteInvoice(int id, CancellationToken cancellationToken)
    {
        ...
        _invoiceStore.RemoveInvoice(invoice);
        await _invoiceStore.SaveChangesAsync(cancellationToken);
    ...
    {
        var total = await _invoiceStore.QueryInvoices().CountAsync(cancellationToken);
        var verified = await _invoiceStore.QueryInvoices().CountAsync(i => i.Status == "verified", cancellationToken);
        var pending = await _invoiceStore.QueryInvoices().CountAsync(i => i.Status == "pending", cancellationToken);
        var rejected = await _invoiceStore.QueryInvoices().CountAsync(i => i.Status == "rejected", cancellationToken);
        var exception = await _invoiceStore.QueryInvoices().CountAsync(i => i.Status == "exception", cancellationToken);

        // 计算平均审核天数
        var reviewedInvoices = await _invoiceStore.QueryInvoices()
