...
public sealed class WorkflowsController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var query = _workflowStore.QueryWorkflowInstances();
    ...
    public async Task<IActionResult> GetWorkflow(int id, CancellationToken cancellationToken)
    {
        ...
        var workflow = await _workflowStore.FindWorkflowInstanceAsync(id, asNoTracking: true, cancellationToken);
        if (workflow == null)
        {
            return NotFound(new { message = "Workflow not found." });
        }

        var steps = await _workflowStore.LoadWorkflowStepsAsync(id, cancellationToken);
    ...
    public async Task<IActionResult> CreateWorkflow([FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        await using var transaction = await _workflowStore.BeginTransactionAsync(cancellationToken);
        try
        {
            _workflowStore.AddWorkflowInstance(workflow);
            ...
            if (stepsPayload.Count > 0)
            {
                ...
                _workflowStore.AddWorkflowSteps(stepEntities);
                await _workflowStore.SaveChangesAsync(cancellationToken);
            }
    ...
    public async Task<IActionResult> UpdateWorkflow(int id, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var workflow = await _workflowStore.FindWorkflowInstanceAsync(id, asNoTracking: false, cancellationToken);
        if (workflow == null)
        {
            return NotFound(new { message = "Workflow not found." });
        }
        ...
        var steps = await _workflowStore.LoadWorkflowStepsAsync(workflow.Id, cancellationToken);
    ...
    public async Task<IActionResult> UpdateWorkflowStep(int id, int stepId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var step = await _workflowStore.FindWorkflowStepAsync(id, stepId, cancellationToken);
        if (step == null)
        {
        ...
        if (workflow == null)
        {
            workflow = await _workflowStore.FindWorkflowInstanceAsync(id, asNoTracking: true, cancellationToken);
        }
    ...
    private async Task<List<object>> BuildWorkflowPayloadsAsync(IReadOnlyList<WorkflowInstance> workflows, CancellationToken cancellationToken)
    {
        ...
        var steps = await _workflowStore.LoadWorkflowStepsAsync(workflowIds, cancellationToken);
