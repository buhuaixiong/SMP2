...
public sealed partial class PermissionsController
{
    ...
    public async Task<IActionResult> GetAllBuyers([FromQuery] bool withStats, CancellationToken cancellationToken)
    {
        ...
        var buyersQuery = _permissionsDataService.QueryUsers()
        ...
        var buyersWithStats = await buyersQuery
            .Select(u => new
            {
                ...
                u.CreatedAt,
                u.Role,
                assignmentCount = _permissionsDataService.QueryBuyerSupplierAssignments(true).Count(a => a.BuyerId == u.Id),
            })
            .ToListAsync(cancellationToken);
    ...
    public async Task<IActionResult> GetBuyerAssignments(string buyerId, CancellationToken cancellationToken)
    {
        ...
        var buyer = await _permissionsDataService.QueryUsers()
        ...
        var assignments = await (from assignment in _permissionsDataService.QueryBuyerSupplierAssignments()
                                 join supplier in _permissionsDataService.QuerySuppliers()
                                     on assignment.SupplierId equals supplier.Id into supplierJoin
                                     from supplier in supplierJoin.DefaultIfEmpty()
                                 join permission in _permissionsDataService.QueryBuyerSupplierPermissions()
        ...
        CancellationToken cancellationToken)
    {
        ...
        var buyer = await _permissionsDataService.QueryUsers(asNoTracking: false).FirstOrDefaultAsync(u => u.Id == buyerId, cancellationToken);
        if (buyer == null)
        {
            return NotFound(new { message = "Buyer not found." });
        }
        ...
        var currentSupplierIds = await _permissionsDataService.QueryBuyerSupplierAssignments()
        ...
        await using var transaction = await _permissionsDataService.BeginTransactionAsync(cancellationToken);
        try
        {
            if (toRemove.Count > 0)
            {
                var removeAssignments = _permissionsDataService.QueryBuyerSupplierAssignments(asNoTracking: false)
                    .Where(a => a.BuyerId == buyerId && toRemove.Contains(a.SupplierId));
                _permissionsDataService.RemoveBuyerSupplierAssignments(removeAssignments);

                var removePermissions = _permissionsDataService.QueryBuyerSupplierPermissions(asNoTracking: false)
                    .Where(p => p.BuyerId == buyerId && toRemove.Contains(p.SupplierId));
                _permissionsDataService.RemoveBuyerSupplierPermissions(removePermissions);
            }
            ...
            if (toAdd.Count > 0)
            {
                ...
                {
                    _permissionsDataService.AddBuyerSupplierAssignment(new BuyerSupplierAssignment
                    {
                        BuyerId = buyerId,
            ...
            {
                var existing = await _permissionsDataService.QueryBuyerSupplierPermissions(asNoTracking: false)
                    .FirstOrDefaultAsync(p => p.BuyerId == buyerId && p.SupplierId == assignment.SupplierId, cancellationToken);

                var now = DateTimeOffset.UtcNow.ToString("o");
                if (existing == null)
                {
                    _permissionsDataService.AddBuyerSupplierPermission(new BuyerSupplierPermission
                    {
                        BuyerId = buyerId,
            ...
            await _permissionsDataService.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IActionResult> GetBuyerStats(string buyerId, CancellationToken cancellationToken)
    {
        ...
        var totalSuppliers = await _permissionsDataService.QueryBuyerSupplierAssignments()
    ...
    public async Task<IActionResult> SearchAssignableSuppliers(CancellationToken cancellationToken)
    {
        ...
        var query = _permissionsDataService.QuerySuppliers()
    ...
    public async Task<IActionResult> GetSupplierAssignedBuyers(int supplierId, CancellationToken cancellationToken)
    {
        ...
        var buyers = await (from assignment in _permissionsDataService.QueryBuyerSupplierAssignments()
                            join user in _permissionsDataService.QueryUsers()
    ...
    public async Task<IActionResult> CreateAssignment([FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var exists = await _permissionsDataService.QueryBuyerSupplierAssignments(asNoTracking: false)
        ...
        _permissionsDataService.AddBuyerSupplierAssignment(assignment);
        await _permissionsDataService.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IActionResult> DeleteAssignment(string buyerId, int supplierId, CancellationToken cancellationToken)
    {
        ...
        var assignment = await _permissionsDataService.QueryBuyerSupplierAssignments(asNoTracking: false)
        ...
        _permissionsDataService.RemoveBuyerSupplierAssignment(assignment);
        await _permissionsDataService.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IActionResult> BatchAssignSuppliers(string buyerId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var existing = await _permissionsDataService.QueryBuyerSupplierAssignments()
    ...
    public async Task<IActionResult> BatchUnassignSuppliers(string buyerId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        var deleteAssignments = _permissionsDataService.QueryBuyerSupplierAssignments(asNoTracking: false)
            .Where(a => a.BuyerId == buyerId && supplierIds.Contains(a.SupplierId));

        _permissionsDataService.RemoveBuyerSupplierAssignments(deleteAssignments);
        var deleted = await _permissionsDataService.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IActionResult> DeleteAllBuyerAssignments(string buyerId, CancellationToken cancellationToken)
    {
        ...
        var assignments = _permissionsDataService.QueryBuyerSupplierAssignments(asNoTracking: false).Where(a => a.BuyerId == buyerId);
        _permissionsDataService.RemoveBuyerSupplierAssignments(assignments);
        var deleted = await _permissionsDataService.SaveChangesAsync(cancellationToken);
    ...
    public async Task<IActionResult> DeleteAllSupplierAssignments(int supplierId, CancellationToken cancellationToken)
    {
        ...
        var assignments = _permissionsDataService.QueryBuyerSupplierAssignments(asNoTracking: false).Where(a => a.SupplierId == supplierId);
        _permissionsDataService.RemoveBuyerSupplierAssignments(assignments);
        var deleted = await _permissionsDataService.SaveChangesAsync(cancellationToken);
