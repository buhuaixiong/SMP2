...
public sealed partial class ContractsController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var query = _contractStore.QueryContracts();
        ...
        var versions = await _contractStore.LoadContractVersionsAsync(contractIds, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var supplierExists = await _contractStore.SupplierExistsAsync(request.SupplierId, cancellationToken);
        if (!supplierExists)
        {
            return NotFound(new { message = "Supplier not found." });
        }

        var agreementExists = await _contractStore.ContractAgreementExistsAsync(request.AgreementNumber, cancellationToken);
        if (agreementExists)
        {
        ...
        _contractStore.AddContract(contract);
    ...
    public async Task<IActionResult> DeleteContract(int id, CancellationToken cancellationToken)
    {
        ...
        _contractStore.RemoveContract(contract);
        await _contractStore.SaveChangesAsync(cancellationToken);
