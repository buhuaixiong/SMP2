...
public sealed partial class RfqWorkflowController
{
    ...
    public async Task<IActionResult> GeneratePrExcel(long id, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var rfqForNotification = await _rfqWorkflowStore.FindRfqAsync(id, asNoTracking: true, cancellationToken);
    ...
    {
        var records = (await _rfqWorkflowStore.LoadLineItemExportRecordsAsync(rfqId, lineItemIds, cancellationToken))
            .Select(row => new LineItemExportRecord
            {
                Id = row.Id,
        ...
        if (itemsNeedingFix.Count > 0)
        {
            ...
            foreach (var item in itemsNeedingFix)
            {
                ...
                var quote = await _rfqWorkflowStore.FindQuoteByIdAsync(item.SelectedQuoteId.Value, asNoTracking: false, cancellationToken);
                if (quote == null)
                {
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var existingPr = await _rfqWorkflowStore.FindRfqPrRecordAsync(rfqId, asNoTracking: true, cancellationToken);
            ...
            if (existingPr != null)
            {
                ...
                if (string.Equals(existingPr.PrNumber, prNumber, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    var completedLineItemIds = await _rfqWorkflowStore.LoadCompletedLineItemIdsAsync(rfqId, cancellationToken);
                    ...
                    // 更新新行项目的状态
                    var lineItemsToUpdate = await _rfqWorkflowStore.LoadRfqLineItemsByIdsAsync(rfqId, newLineItemIds, cancellationToken);
            ...
            _rfqWorkflowStore.AddRfqPrRecord(record);

            if (exportedLineItemIds.Count > 0)
            {
                var lineItemsToUpdate = await _rfqWorkflowStore.LoadRfqLineItemsByIdsAsync(rfqId, exportedLineItemIds, cancellationToken);
