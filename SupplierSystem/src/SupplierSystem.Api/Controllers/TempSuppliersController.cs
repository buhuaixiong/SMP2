...
public sealed class TempSuppliersController : ControllerBase
{
    ...
    public async Task<IActionResult> ListPendingApplications(
        ...
        CancellationToken cancellationToken = default)
    {
        return await WithAuthValidation(async user =>
        {
            try
            {
                var applications = await _upgradeService.ListPendingApplicationsAsync(user, page, pageSize, cancellationToken);
