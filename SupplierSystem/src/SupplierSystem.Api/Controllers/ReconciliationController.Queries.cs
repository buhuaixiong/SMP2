...
public sealed partial class ReconciliationController
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var reconciliation = await _reconciliationStore.FindReconciliationAsync(reconciliationId.Value, cancellationToken);
        ...
        var receipts = await _reconciliationStore.LoadWarehouseReceiptsBySupplierAsync(supplierId, cancellationToken);
        if (reconciliationId.HasValue)
        {
            var receiptId = await _reconciliationStore.LoadWarehouseReceiptIdForReconciliationAsync(reconciliationId.Value, cancellationToken);
            receipts = receipts.Where(r => r.Id == receiptId).ToList();
        }
        ...
        var receiptDetails = await _reconciliationStore.LoadReceiptAggregatesAsync(receiptIds, cancellationToken);
        var detailMap = receiptDetails.ToDictionary(d => d.WarehouseReceiptId);

        var reconciliationMap = await _reconciliationStore.LoadReconciliationsByReceiptIdsAsync(receiptIds, cancellationToken);
    ...
    public async Task<IActionResult> GetReconciliationById(int id, CancellationToken cancellationToken)
    {
        ...
        var reconciliation = await _reconciliationStore.FindReconciliationAsync(id, cancellationToken);
        ...
        var variance = (await _reconciliationStore.LoadVarianceAnalysesAsync(id, cancellationToken)).FirstOrDefault();
        if (variance != null)
        {
        ...
        var statusHistory = await _reconciliationStore.LoadStatusHistoriesAsync(id, cancellationToken);
        ...
        var matches = await _reconciliationStore.LoadMatchesAsync(id, cancellationToken);
    ...
    private async Task<List<Reconciliation>> QueryReconciliationsAsync(
        ...
        CancellationToken cancellationToken)
    {
        var query = _reconciliationStore.QueryReconciliations();
        ...
        CancellationToken cancellationToken)
    {
        ...
        var suppliers = await _reconciliationStore.LoadSupplierNameMapAsync(supplierIds, cancellationToken);
