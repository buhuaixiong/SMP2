...
public sealed class AuthController : ApiControllerBase
{
    ...
    private readonly AuthReadStore _authReadStore;

    public AuthController(
        IAuthService authService,
        IAccountLockoutService lockoutService,
        ISessionService sessionService,
        AuthReadStore authReadStore)
    {
    ...
    public async Task<IActionResult> GetBlacklistStats([FromQuery] bool details, CancellationToken cancellationToken)
    {
        ...
        var total = await _authReadStore.QueryTokenBlacklist().CountAsync(cancellationToken);
        var active = await _authReadStore.QueryTokenBlacklist().CountAsync(
            entry => string.Compare(entry.ExpiresAt, now) > 0,
            cancellationToken);
        var expired = await _authReadStore.QueryTokenBlacklist().CountAsync(
            entry => string.Compare(entry.ExpiresAt, now) <= 0,
            cancellationToken);

        var reasonStats = await _authReadStore.QueryTokenBlacklist()
        ...
        if (details)
        {
            var recentTokens = await _authReadStore.QueryTokenBlacklist()
                .Where(entry => string.Compare(entry.ExpiresAt, now) > 0)
    ...
    public async Task<IActionResult> GetInvitation(string token, CancellationToken cancellationToken)
    {
        ...
        var invitation = await _authReadStore.GetInvitationDetailsAsync(token, cancellationToken);
