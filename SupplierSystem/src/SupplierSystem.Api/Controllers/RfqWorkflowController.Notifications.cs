...
public sealed partial class RfqWorkflowController
{
    ...
    {
        var combined = await _rfqWorkflowStore.LoadSupplierEmailsForRfqAsync(rfqId, cancellationToken);
    ...
    private async Task<List<string>> ResolveRoleEmailsAsync(string role, CancellationToken cancellationToken)
    {
        ...
        var emails = await _rfqWorkflowStore.LoadUserEmailsByRoleAsync(normalizedRole, cancellationToken);
    ...
    {
        var config = await _rfqWorkflowStore.FindSystemConfigAsync(SmtpConfigKey, cancellationToken);
