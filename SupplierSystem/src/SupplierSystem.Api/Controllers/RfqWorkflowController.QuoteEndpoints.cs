...
public sealed partial class RfqWorkflowController
{
    ...
    public async Task<IActionResult> SubmitQuote(int id, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                return BadRequest(new { message = "RFQ has no active bid round." });
            }
            ...
            {
                return BadRequest(new { message = "The current bidding round is not accepting quotes." });
            }

            var roundDeadline = currentBidRound.BidDeadline ?? rfq.ValidUntil;
            if (!string.IsNullOrWhiteSpace(roundDeadline) &&
                DateTime.TryParse(roundDeadline, out var deadline) &&
                deadline < DateTime.UtcNow)
            {
                return BadRequest(new { message = "The current bidding round has expired." });
            }
            ...
            {
                return BadRequest(new { message = "The current bidding round has already been opened and can no longer accept quote changes." });
            }
            ...
            var hasQuote = await _rfqWorkflowStore.HasQuoteAsync(id, user.SupplierId.Value, currentBidRound.Id, cancellationToken);
            if (hasQuote)
            {
            ...
            var rfqLineItems = await _rfqWorkflowStore.LoadRfqLineItemQuantitiesAsync(id, lineItemIds, cancellationToken);
            ...
            _rfqWorkflowStore.AddQuote(quote);
            ...
            foreach (var item in lineItems)
            {
                ...
                _rfqWorkflowStore.AddQuoteLineItem(new QuoteLineItem
                {
                    QuoteId = quote.Id,
            ...
            foreach (var file in storedFiles)
            {
                ...
                _rfqWorkflowStore.AddQuoteAttachment(new QuoteAttachment
                {
                    QuoteId = quote.Id,
    ...
    public async Task<IActionResult> GetQuotes(int id, CancellationToken cancellationToken)
    {
        ...
        {
            var currentBidRound = await _rfqWorkflowStore.FindCurrentBidRoundAsync(id, asNoTracking: true, cancellationToken);
            var invited = await _rfqWorkflowStore.FindInvitationAsync(id, user.SupplierId!.Value, currentBidRound?.Id, cancellationToken) != null;
            if (!invited)
            {
        ...
        {
            var currentBidRound = await _rfqWorkflowStore.FindCurrentBidRoundAsync(id, asNoTracking: true, cancellationToken);
            var quoteQuery = _rfqWorkflowStore.QueryQuoteListRows(id, currentBidRound?.Id);
    ...
    [HttpPost("{id:int}/quotes/{quoteId:int}")]
    public async Task<IActionResult> UpdateQuote(int id, int quoteId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var quote = await _rfqWorkflowStore.FindQuoteForSupplierAsync(quoteId, id, user.SupplierId.Value, asNoTracking: false, cancellationToken);
            if (quote == null)
            {
            ...
            var currentBidRound = await _rfqWorkflowStore.FindCurrentBidRoundAsync(id, asNoTracking: true, cancellationToken);
            if (currentBidRound == null || quote.BidRoundId != currentBidRound.Id)
            {
                return BadRequest(new { message = "Only quotes from the current bidding round can be updated." });
            }
            ...
            if (!string.IsNullOrWhiteSpace(currentBidRound.BidDeadline ?? rfq.ValidUntil) &&
                DateTime.TryParse(currentBidRound.BidDeadline ?? rfq.ValidUntil, out var deadline) &&
                deadline < DateTime.UtcNow)
            {
            ...
            var existingLineItems = await _rfqWorkflowStore.LoadQuoteLineItemsAsync(quoteId, cancellationToken);
            _rfqWorkflowStore.RemoveQuoteLineItems(existingLineItems);
    ...
    public async Task<IActionResult> WithdrawQuote(int id, int quoteId, [FromBody] JsonElement body, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (currentBidRound == null || quote.BidRoundId != currentBidRound.Id)
            {
                return BadRequest(new { message = "Only quotes from the current bidding round can be withdrawn." });
            }
