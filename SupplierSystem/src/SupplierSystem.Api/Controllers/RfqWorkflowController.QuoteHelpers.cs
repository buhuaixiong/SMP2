...
public sealed partial class RfqWorkflowController
{
    ...
    private static string NormalizeInvitedSupplierQuoteStatus(string? status)
    {
        var normalized = status?.Trim().ToLowerInvariant();
        return normalized switch
        {
            "submitted" or "under_review" or "selected" or "rejected" => "submitted",
            "withdrawn" => "withdrawn",
            _ => "not_submitted",
        };
    }
    ...
    {
        var quote = await _rfqWorkflowStore.FindQuoteByIdAsync(quoteId, asNoTracking: true, cancellationToken);
        if (quote == null)
        {
            return null;
        }
    ...
    private async Task<List<Dictionary<string, object?>>> GetQuoteLineItemsAsync(
        ...
        CancellationToken cancellationToken)
    {
        var rows = await _rfqWorkflowStore.LoadQuoteLineItemRowsAsync(quoteId, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var rfqId = await _rfqWorkflowStore.FindRfqIdForQuoteAsync(quoteId, cancellationToken);
            var rows = await _rfqWorkflowStore.LoadQuoteAttachmentsAsync(quoteId, cancellationToken);
