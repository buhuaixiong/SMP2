...
public sealed partial class TempSupplierUpgradeService
{
    ...
    public async Task<IReadOnlyList<object>> ListPendingApplicationsAsync(
        AuthUser user,
        int page,
        int pageSize,
        CancellationToken cancellationToken)
    {
        ...
        if (IsPurchaser(user))
        {
            var assignedSupplierIds = await GetAssignedSupplierIdsAsync(user, cancellationToken);
            if (assignedSupplierIds.Count == 0)
            {
                return Array.Empty<object>();
            }

            applications = await _repository.ListPendingApplicationsBySupplierIdsAsync(
                assignedSupplierIds,
                offset,
                normalizedPageSize,
                cancellationToken);
        ...
        {
            applications = await _repository.ListPendingApplicationsAsync(offset, normalizedPageSize, cancellationToken);
        }

        var documentsByApplication = await LoadDocumentsByApplicationIdAsync(applications, cancellationToken);
        var result = new List<object>(applications.Count);
        foreach (var app in applications)
        {
            documentsByApplication.TryGetValue(app.Id, out var documents);
            var applicationDocuments = documents ?? new List<UpgradeDocumentDetail>();
            result.Add(new
            {
                id = app.Id,
                ...
                dueAt = app.DueAt,
                documentCompleteness = CalculateCompleteness(applicationDocuments),
                documents = applicationDocuments.Select(BuildDocumentView).ToList()
            });
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return applications.Where(a => assignedSupplierIds.Contains(a.SupplierId)).ToList();
    }

    private async Task<HashSet<int>> GetAssignedSupplierIdsAsync(AuthUser user, CancellationToken cancellationToken)
    {
        ...
        return new HashSet<int>(assignedSupplierIds);
    }

    private async Task<Dictionary<int, List<UpgradeDocumentDetail>>> LoadDocumentsByApplicationIdAsync(
        IReadOnlyCollection<PendingUpgradeApplicationRecord> applications,
        CancellationToken cancellationToken)
    {
        ...
        {
            return new Dictionary<int, List<UpgradeDocumentDetail>>();
        }

        var documents = await _repository.GetApplicationDocumentsByApplicationIdsAsync(applicationIds, cancellationToken);
        return documents
            .GroupBy(document => document.ApplicationId)
