...
public sealed class TempSupplierUpgradeRepository
{
    ...
    public async Task<List<PendingUpgradeApplicationRecord>> ListPendingApplicationsAsync(
        int offset,
        int limit,
        CancellationToken cancellationToken)
    {
...
WHERE sua.status LIKE 'pending_%'
...
OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY;";
    ...
    public async Task<List<PendingUpgradeApplicationRecord>> ListPendingApplicationsBySupplierIdsAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var parameterName = $"@supplierId{index}";
            parameterNames.Add(parameterName);
            AddParameter(command, parameterName, ids[index]);
        }
...
INNER JOIN suppliers s ON s.id = sua.supplier_id
WHERE sua.status LIKE 'pending_%'
  AND sua.supplier_id IN ({string.Join(", ", parameterNames)})
ORDER BY sua.submittedAt ASC
OFFSET @offset ROWS FETCH NEXT @limit ROWS ONLY;";
        AddParameter(command, "@offset", normalizedOffset);
        AddParameter(command, "@limit", normalizedLimit);
...
SELECT CAST(SCOPE_IDENTITY() as int);";
    ...
    public async Task<List<UpgradeDocumentDetail>> GetApplicationDocumentsByApplicationIdsAsync(
        IReadOnlyCollection<int> applicationIds,
        CancellationToken cancellationToken)
    {
        if (applicationIds == null || applicationIds.Count == 0)
        {
            return new List<UpgradeDocumentDetail>();
        }
        ...
        for (var index = 0; index < ids.Count; index++)
        {
            var parameterName = $"@applicationId{index}";
            parameterNames.Add(parameterName);
            AddParameter(command, parameterName, ids[index]);
        }
...
SELECT d.id, d.applicationId, d.requirementCode, d.requirementName, d.fileId,
       d.uploadedAt, d.uploadedBy, d.status, d.notes, f.originalName, f.storedName, f.fileType, f.uploadTime, f.validFrom, f.validTo
FROM supplier_upgrade_documents d
LEFT JOIN files f ON f.id = d.fileId
WHERE d.applicationId IN ({string.Join(", ", parameterNames)})
ORDER BY d.applicationId ASC, d.uploadedAt ASC;";

        return SqlServerHelper.ReadAll(command).Select(MapDocumentDetail).ToList();
    }
