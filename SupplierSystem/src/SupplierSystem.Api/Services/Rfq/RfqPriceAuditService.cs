...
public sealed class RfqPriceAuditService
{
    ...
    public async Task UpsertQuoteAuditAsync(Quote quote, string? ipAddress, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var bidRound = quote.BidRoundId.HasValue
                ? await _dbContext.RfqBidRounds.AsNoTracking()
                    .FirstOrDefaultAsync(round => round.Id == quote.BidRoundId.Value, cancellationToken)
                : null;
            ...
            foreach (var qli in quoteLineItems)
            {
                ...
                var record = existingMap.TryGetValue(qli.RfqLineItemId, out var existingRecord)
                    ...
                    {
                        RfqId = quote.RfqId,
                        BidRoundId = quote.BidRoundId,
                        RoundNumber = bidRound?.RoundNumber,
                ...
                record.BidRoundId = quote.BidRoundId;
                record.RoundNumber = bidRound?.RoundNumber;
        ...
        CancellationToken cancellationToken)
    {
        ...
        var record = await _dbContext.RfqPriceAuditRecords
            .FirstOrDefaultAsync(r => r.RfqId == rfq.Id &&
                                      r.BidRoundId == quote.BidRoundId &&
                                      r.RfqLineItemId == null &&
                                      r.SupplierId == quote.SupplierId,
                cancellationToken);
