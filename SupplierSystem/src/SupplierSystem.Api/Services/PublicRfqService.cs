...
public sealed record PublicAutoLoginUser(string Id, string Name, string Role, int? SupplierId, bool MustChangePassword);
...
public sealed class PublicRfqService
{
    ...
    {
        if (!string.IsNullOrWhiteSpace(envValue) && !IsPlaceholder(envValue))
        {
            return envValue;
        }
