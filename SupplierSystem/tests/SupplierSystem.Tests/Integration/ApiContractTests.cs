...
public sealed class ApiContractTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    [Fact]
    public async Task OpenApi_ContainsPostMethod_ForPasswordResetEndpoint()
    {
        using var client = _factory.CreateClient();
        using var response = await client.GetAsync("/openapi/v1.json");
        ...
        Assert.True(
            openApiPaths.TryGetValue("/api/permissions/users/{}/password", out var methods),
            "Expected path missing from OpenAPI: /api/permissions/users/{}/password");
